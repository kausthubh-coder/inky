...
it.each(["all", "selected"])("normalizes toolkit case without bypassing %s policy", async (access) => {
  ...
  vi.stubEnv(
    "STUDI_COMPOSIO_TOOL_POLICY_JSON",
    JSON.stringify({
      gmail: {
        version: "20260902_00",
        ...(access === "all" ? { access } : { tools: ["GMAIL_FETCH_EMAILS"] }),
      },
    }),
  );
  ...
  const result = await t
    .withIdentity({ subject: "composio_test" })
    .action(api.composio.search, { toolkit: "gmail", query: "find emails" });
  expect(result.tools.map((tool) => tool.slug)).toEqual(
    access === "all" ? ["GMAIL_FETCH_EMAILS", "GMAIL_CREATE_EMAIL_DRAFT"] : ["GMAIL_FETCH_EMAILS"],
  );
...
it("refuses anonymous and unapproved callers before contacting Composio", async () => {
  ...
  await expect(
    t.withIdentity({ subject: "unapproved_test" }).action(api.composio.search, args),
  ).rejects.toThrow();
