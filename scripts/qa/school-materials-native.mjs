...
const server = createServer((request, response) => {
  if (request.url === "/" || request.url === "/work") {
    ...
    response.end(
      '<h1>Exercise 6</h1><p>Read the description and save answer.txt in your assignment folder. Keep the source files for review. No upload is required.</p><a href="/redirect">Exercise 6 Description PDF</a><a href="/starter.txt">Starter file</a>' +
        (request.url === "/" ? '<a href="/login">Unavailable attachment</a>' : ""),
    );
    return;
  }
  if (!request.headers.cookie?.includes("qa_school=allowed") || request.url === "/login") {
    response.setHeader("content-type", "text/html");
    response.end("<h1>Please sign in</h1>");
    return;
  }
  if (request.url === "/redirect") {
    response.writeHead(302, { location: "/exercise.pdf" });
    response.end();
    return;
  }
...
try {
  ...
  window.webContents.on("did-start-navigation", (_event, _url, _inPlace, main) => {
    if (main) browser.pageChanged();
  });
  ...
  for (const [label, result] of [
    ...
    await writeFile(
      join(evidence, `${label}.png`),
      Buffer.from(result.content.find((c) => c.type === "image").data, "base64"),
    );
  }
  snapshot = await browser.snapshot();
  const starter = await download.execute("starter", {
    ref: snapshot.elements.find((e) => e.name === "Starter file").ref,
  });
  ...
  await assert.rejects(
    download.execute("login", {
      ref: snapshot.elements.find((e) => e.name === "Unavailable attachment").ref,
    }),
    /web page/,
  );
  ...
  try {
    ...
    const assignment = {
      ...
      courseId: "scan-course",
      title: "Exercise 6",
      sourceTarget: `${origin}/work`,
      discoveredAt: capturedAt,
      evidence: [],
    };
    scanStore.assignments.put(assignment);
    let active = true;
    const scanPdf = createScanMaterialReader({
      store: scanStore,
      browser,
      now: () => capturedAt,
    ...
    const first = await scanPdf.tool.execute("scan-pdf", {
      assignmentId: assignment.assignmentId,
      ref: linked.elements.find((e) => e.name === "Exercise 6 Description PDF").ref,
    });
    ...
    assert.throws(
      () => scanPdf.resolveExcerpt(first.details.sourceRef, assignment.assignmentId, "invented instruction"),
      /Quote the attachment/,
    );
    assert.throws(
      () => scanPdf.resolveExcerpt(first.details.sourceRef, "other-assignment", "Add 2 and 3"),
      /this assignment/,
    );
    const second = await scanPdf.tool.execute("scan-page-two", {
      assignmentId: assignment.assignmentId,
      documentId: first.details.documentId,
      page: 2,
      image: true,
    });
    ...
    assert.match(
      scanPdf.resolveExcerpt(second.details.sourceRef, assignment.assignmentId, "Transcribed image text")
        .summary,
      /Visual transcription/,
    );
    const textLink = await browser.snapshot();
    const plain = await scanPdf.tool.execute("scan-text", {
      assignmentId: assignment.assignmentId,
      ref: textLink.elements.find((e) => e.name === "Starter file").ref,
    });
    assert.match(plain.content[0].text, /starter = 2 \+ 3/);
    assert.equal(
      scanPdf.resolveExcerpt(plain.details.sourceRef, assignment.assignmentId, "starter = 2 + 3").kind,
      "document",
    );
    active = false;
    await assert.rejects(
      scanPdf.tool.execute("stopped", {
        assignmentId: assignment.assignmentId,
        documentId: first.details.documentId,
      }),
      /Scan stopped/,
  ...
  try {
    await browser.navigate(`${origin}/exercise.pdf`);
  } catch (error) {
  ...
  try {
    const screenshot = await createBrowserTools(browser)
  ...
  const receipt = {
    mode: "controlled native Electron",
    ...
    viewerCapture,
    passed: [
      "signed-in download from observed link",
      "redirect with session cookie",
      "duplicate preservation",
      "starter file",
      "PDF text and image-only page",
      "stale link rejection",
      "HTML/login rejection",
      "current-document download",
      "scan PDF/text provenance and assignment isolation",
      "scan image-page transcription and cancellation",
    ],
