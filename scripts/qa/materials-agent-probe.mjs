...
export async function runMaterialsAgentProbe(browser, origin, root, evidence) {
    ...
    onDiagnostic(event) {
      if (event.kind === "session_created")
        trace.push({
          ...
          reasoning: event.payload.reasoning_effort,
          tools: event.payload.tools.map((t) => t.name),
        });
      if (event.kind === "tool_execution_start") {
        trace.push({ kind: event.kind, name: event.payload.toolName });
        console.log(`Agent tool: ${event.payload.toolName}`);
      }
      if (event.kind === "tool_execution_end")
        trace.push({ kind: event.kind, name: event.payload.toolName, isError: event.payload.isError });
      if (event.kind === "generation" && event.payload.$ai_is_error)
        trace.push({ kind: "provider_error", error: event.payload.$ai_error });
    },
  });
  const provider = await runtime.getProviderStatus("openai-codex");
  if (provider.state !== "ready") {
    await writeFile(
      join(evidence, "agent.json"),
      JSON.stringify({ status: "blocked", provider: provider.state }, null, 2),
    );
    store.close();
    console.log(`Live agent unavailable: ${provider.state}`);
    return;
  }
  const assignmentId = "qa-pdf";
  const taskId = "qa-pdf-task";
  ...
  const timeout = setTimeout(() => {
    void manager.pauseForStudent(taskId, "QA time limit").catch(() => {});
  }, 120_000);
  let result;
  try {
    ...
    result = {
      status: "passed",
      mode: "live provider, production coordinator, controlled local school",
      phase: execution.phase,
      answer: answer.content,
      materials: materials.map((f) => f.path),
      trace,
    };
    ...
    result = {
      status: "failed",
      error: error.message,
      phase: coordinator.state(false).execution?.phase,
      blocker: coordinator.state(false).execution?.lastError,
      trace,
    };
    ...
    try {
      ...
      /* A failed task has already released its worker. */
    }
    coordinator.dispose();
    store.close();
  }
