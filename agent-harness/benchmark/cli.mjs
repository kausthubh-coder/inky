...
const { values, positionals } = parseArgs({
  ...
  options: {
    ...
    output: { type: "string" },
    "budget-ms": { type: "string", default: "60000" },
    "max-tool-calls": { type: "string", default: "120" },
    "lms-module": { type: "string" },
    ...
    phases: { type: "string", default: "cold" },
...
if (positionals[0] === "live") {
  ...
  const { path, record } = await runLive({
    lmsModule: values["lms-module"],
    buildRoot: values.build,
    gitSha: values.revision,
  ...
  console.log(
    JSON.stringify({
      ...
      phases: record.phases.map((phase) => ({
        name: phase.name,
        status: phase.status,
        scanState: phase.scanState,
        passed: phase.grade.passed,
        metrics: phase.metrics,
      })),
    }),
  );
  process.exitCode =
    record.phases.length === record.config.phases.length && record.phases.every((phase) => phase.grade.passed)
      ? 0
      : 1;
} else if (positionals[0] === "replay") {
  const { path, record } = await runReplay({
    buildRoot: values.build,
    gitSha: values.revision,
    budgetMs: Number(values["budget-ms"]),
    maxToolCalls: Number(values["max-tool-calls"]),
  ...
  console.log(
    JSON.stringify({
      ...
      status: phase.status,
      passed: phase.grade.passed,
      checks: phase.grade.checks.length,
      failed: phase.grade.checks.filter((item) => !item.passed),
    }),
  );
  ...
  const [baseline, candidate] = await Promise.all(
    positionals.slice(1, 3).map(async (path) => JSON.parse(await readFile(path, "utf8"))),
  );
  ...
  throw new Error(
    "Usage: bun run benchmark -- replay [--build dist] | compare baseline.json candidate.json [--output comparison.json]",
  );
