...
const trace = (event) => {
  ...
  traceWrites = traceWrites.then(() =>
    appendFile(join(config.runRoot, "trace.jsonl"), JSON.stringify(safe) + "\n"),
  );
};
function assertBudget() {
  if (exhausted || calls > config.maxToolCalls) throw new Error("Run-wide tool-call budget exceeded");
}
function allowed(url) {
  try {
    return url === "about:blank" || allowedOrigins.has(new URL(url).origin);
  } catch {
    return false;
  }
}
...
async function initialize() {
  ...
  window = new BrowserWindow({
    ...
    webPreferences: {
      ...
      offscreen: !config.show,
      partition: `benchmark-${config.runId}`,
    },
  });
  ...
  window.webContents.session.webRequest.onBeforeRequest((details, callback) => {
    ...
    if (!allowed(details.url) && !internalPdf) {
      violations.push("Attempted navigation outside the fixture");
      trace({ kind: "forbidden_navigation", url: details.url });
    }
  ...
  const browser = new BrowserController({
    ...
    loadURL: async (url) => {
      if (!allowed(url)) {
        violations.push("Attempted navigation outside the fixture");
        trace({ kind: "forbidden_navigation", url });
        throw new Error("Only this run's fake school is allowed");
      }
      ...
      return await Promise.race([
        ...
        new Promise((_, reject) => {
          timer = setTimeout(
            () =>
              reject(
                new Error("Screenshot unavailable in the benchmark window; use the accessibility snapshot."),
              ),
            5000,
          );
        }),
  ...
  runtime = await PiAgentRuntime.create({
  ...
  const scanRuntime = {
    createScanSession: async (tools, target, control) => {
      activeSession = await runtime.createScanSession(tools, target, {
        assertActive: () => {
          assertBudget();
      ...
      trace({
        kind: "effective_session",
        model: runtime.selectedModelId,
        effort: runtime.selectedReasoningEffort,
        toolNames: activeSession.toolNames,
      });
      return activeSession;
    },
  };
  coordinator = new SchoolScanCoordinator(store, scanRuntime, browser, {
    manager,
    now: () => config.clock,
...
async function runPhase(name) {
  ...
  const generations = phaseEvents.filter(
    (event) => event.kind === "diagnostic" && event.diagnostic.kind === "generation",
  );
  const total = (key) =>
    generations.every((event) => typeof event.diagnostic.payload[key] === "number")
      ? generations.reduce((sum, event) => sum + event.diagnostic.payload[key], 0)
      : null;
  const usage = generations.length
    ? {
        inputTokens: total("$ai_input_tokens"),
        outputTokens: total("$ai_output_tokens"),
        cacheReadTokens: total("$ai_cache_read_input_tokens"),
        cacheWriteTokens: total("$ai_cache_creation_input_tokens"),
      }
    : null;
  const result = {
    name,
    status: error ? (exhausted ? "budget_exceeded" : "failed") : "completed",
    error,
    scanState: state.scan?.state ?? null,
...
process.on("message", (message) => {
  chain = chain
    .then(() =>
      message.command === "run" ? runPhase(message.phase) : message.command === "close" ? close() : undefined,
    )
    .catch(async (error) => {
      await traceWrites.catch(() => {});
      process.send?.({ type: "error", error: stripSecrets(error.message) });
      app.exit(1);
    });
});
