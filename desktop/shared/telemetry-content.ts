...
const secretPropertyName =
  /^(?:authorization|password|cookie|set-cookie|token|client[_-]?secret|api[_-]?key|access[_-]?token|refresh[_-]?token|oauth[_-]?code|device[_-]?code|clerk[_-]?token|provider[_-]?credential|verificationUriComplete)$/i;
  ...
  const result = Array.isArray(value)
    ...
    : Object.fromEntries(
        Object.entries(value)
          ...
          .map(([key, item]) => [
            key,
            secretPropertyName.test(key) ? "[secret]" : sanitizeTelemetryValue(item, seen, depth + 1),
          ]),
      );
  seen.delete(value);
  return result;
