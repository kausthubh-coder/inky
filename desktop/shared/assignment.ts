...
export function assignmentWorkEligibility(assignment: Assignment, now: string): AssignmentWorkEligibility {
  ...
  if (!status || status.state === "unknown")
    return blocked("Check whether this assignment is already submitted.");
  if (status.state === "submitted" || status.state === "graded")
    return blocked("The school already records this work as submitted or graded.");
  ...
  if (
    assignment.requirementsState !== "complete" ||
  ...
  if (!assignment.requirementEvidence.every((item) => fresh(item.evidence)))
    return blocked("Refresh the assignment instructions and attached materials before starting.");
  if (
    assignment.deadlinePrecision !== "datetime" ||
    !assignment.dueAt ||
    !fresh(assignment.deadlineEvidence)
  ) {
  ...
  if (Date.parse(assignment.dueAt) <= currentTime) {
    ...
    if (
      ...
      !fresh(late.evidence) ||
      !late.until ||
      Date.parse(late.until) <= currentTime
    ) {
  ...
  return {
    eligible: true,
    reason: "Current school evidence shows unfinished work with requirements and an open deadline.",
  };
}
