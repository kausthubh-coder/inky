...
interface PreviewEnvironment {
  ...
  SchoolPage: ComponentType<{ mode: "classes" | "assignment" }>;
}
let environment: PreviewEnvironment | null = null;
export function installPreviewEnvironment(value: PreviewEnvironment): void {
  environment = value;
}
export function readDevPreviewConfig(): DevPreviewConfig | null {
  return environment?.config ?? null;
}
export function readPreviewSchoolPage() {
  return environment?.SchoolPage ?? null;
}
