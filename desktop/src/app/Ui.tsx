          ...
          {accountOpen ? (
            <div className="account-menu" role="menu" aria-label="Profile menu">
              <ProfileMenuItem
                icon="usage"
                label="Usage"
                active={screen === "settings" && settingsLanding === "usage"}
                onClick={() => openSettings("usage")}
              />
              <ProfileMenuItem
                icon="invite"
                label="Invite friend"
                onClick={() => setAccountNotice("Invite friends is coming soon.")}
              />
              <ProfileMenuItem
                icon="feedback"
                label="Feedback"
                active={screen === "settings" && settingsLanding === "feedback"}
                onClick={() => openSettings("feedback")}
              />
              <ProfileMenuItem
                icon="settings"
                label="Settings"
                active={screen === "settings" && settingsLanding === "settings"}
                onClick={() => openSettings("settings")}
              />
              <ProfileMenuItem
                icon="logout"
              ...
              />
              {accountNotice ? (
                <p className="account-menu__notice" role="status">
                  {accountNotice}
                </p>
              ) : null}
            </div>
...
function ProfileMenuItem({
  ...
  return (
    ...
    >
      ...
      <span>{label}</span>
      {danger ? null : (
        <span className="account-menu__arrow" aria-hidden="true">
          ›
        </span>
      )}
    </button>
  ...
  const paths = {
    ...
    invite: (
      <>
        ...
        <path d="M2.75 13c.45-2.2 1.55-3.25 3.25-3.25S8.8 10.8 9.25 13" />
        <path d="M12 4v4M10 6h4" />
      </>
    ),
    feedback: (
      <>
        <path d="M3 3.5h10v7H7l-3.5 2v-2H3z" />
    ...
    settings: (
      <>
        ...
        <path d="M8 2.5v1.1M8 12.4v1.1M2.5 8h1.1M12.4 8h1.1M4.1 4.1l.8.8M11.1 11.1l.8.8M11.9 4.1l-.8.8M4.9 11.1l-.8.8" />
      </>
    ),
    logout: (
      <>
        <path d="M7 3H3.5v10H7" />
        <path d="M9.5 5.5 12 8l-2.5 2.5M6 8h6" />
      </>
    ),
...
export function PaperCard({
  ...
  id?: string;
  tone?: "paper" | "yellow" | "coral" | "mint" | "sky" | "pink" | "lavender";
  className?: string;
  children: ReactNode;
}) {
  return (
    <section id={id} className={`paper-card tone-${tone} ${className}`}>
      {children}
    </section>
  );
}
...
export function StatusPill({
  ...
  children: ReactNode;
  tone?: "plain" | "mint" | "yellow" | "coral" | "pink" | "sky";
}) {
...
export function ProviderLoginHandoffView({
  login,
  busy,
  onCompleteLogin,
  onCancelLogin,
  ...
  if (login.phase === "starting") {
    return (
      <div className="provider-login">
        ...
        <div>
          <strong>Opening the {name} sign-in…</strong>
        </div>
      </div>
    );
  }
  if (login.phase === "failed" || login.phase === "expired") {
    return (
      <div className="provider-login">
        <div>
          <strong>{login.phase === "expired" ? "That sign-in expired." : "That sign-in didn't work."}</strong>
          <small>Try once more.</small>
        </div>
        {onRetryLogin && (
          <button type="button" className="button button--yellow" onClick={onRetryLogin} disabled={busy}>
            Try again
          </button>
        )}
        {onCancelLogin && (
          <button type="button" className="button" onClick={onCancelLogin} disabled={busy}>
            Dismiss
          </button>
        )}
      </div>
  ...
  const copyCode = async () => {
    ...
    try {
      ...
      /* The student can still read the code. */
    }
  };
    ...
    <div className="provider-login provider-login--code">
      {login.phase === "waiting" ? (
        <>
          ...
          <p className="provider-code" data-secret>
            {login.userCode}
          </p>
          <p>
        ...
        <>
          ...
          {onCompleteLogin && (
            <form className="provider-paste" onSubmit={submitCode}>
              <label>
                Didn't come back here? Paste the code from that page.
                <input
                  data-secret
                  value={code}
      ...
      <div className="provider-login-actions">
        {login.phase === "waiting" && (
          <button type="button" className="button" onClick={() => void copyCode()} disabled={busy}>
...
export function RuntimeAttentionBanner({
  ...
  const providerName = login
    ? agentProviderName(login.providerId)
    : workspace
    ...
    <PaperCard tone="lavender" className="settings-card">
      <div className="card-heading">
        <div>
          ...
          <h3>What I may share</h3>
        </div>
        <StatusPill tone={configured ? "mint" : "yellow"}>
          {configured ? "configured" : "local only"}
        </StatusPill>
      </div>
      <label className="toggle-row">
        <input
          ...
          onChange={(event) => onChange(event.target.checked, telemetry?.replayEnabled ?? true)}
        />
        <span>
          <strong>Share product events</strong>
          <small>
            Private beta includes messages, answers, tool activity, performance, and errors. Passwords,
            cookies, and credentials stay out.
          </small>
        </span>
      </label>
      <label className="toggle-row">
        <input
          type="checkbox"
          checked={telemetry?.replayEnabled ?? false}
          disabled={!configured || busy || !telemetry?.enabled}
          onChange={(event) => onChange(telemetry?.enabled ?? false, event.target.checked)}
        />
        <span>
          <strong>Share Studi replay</strong>
          <small>Records the Studi window, not the school page.</small>
        </span>
      </label>
      <button
        ...
        type="button"
        disabled={!configured || busy || !telemetry?.enabled}
        onClick={() => onDebug(telemetry?.debugUntil ? 0 : 30)}
      >
        {telemetry?.debugUntil ? "Stop beta debug" : "Enable beta debug for 30 minutes"}
      </button>
      <details>
        <summary>Local inspector ({telemetry?.inspector.length ?? 0})</summary>
        <div className="inspector">
          {(telemetry?.inspector.length ?? 0) === 0 ? (
            <small>No upload-eligible envelopes yet.</small>
          ) : (
            telemetry?.inspector
              .slice(-4)
              .reverse()
              .map((item) => <code key={`${item.capturedAt}-${item.event}`}>{JSON.stringify(item)}</code>)
          )}
        </div>
      </details>
    </PaperCard>
...
export function executionLabel(phase: string): string {
  return (
    (
      {
        working: "working visibly",
        needs_user: "waiting for you",
        ready_review: "ready for review",
        submitting: "verifying submission",
        submitted: "submitted",
        preserved: "saved locally",
        failed: "stopped",
