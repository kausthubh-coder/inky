  ...
  useEffect(() => {
    ...
    if (artifactId)
      void window.studi
        ...
        .then((document) => {
          if (!document)
            throw new Error("This saved answer is no longer available.");
  ...
  return (
    <div className="assignment-work">
      ...
      {Boolean(execution?.completionChecklist?.length) && (
        <section
          className="assignment-checklist"
          aria-label="Completed requirements"
        >
      ...
      {state === "preserved" && (
        <p className="assignment-empty-copy">
          Your answers are saved. Submission hasn’t been confirmed.
        </p>
      )}
      {state === "submitted" && (
      ...
      {state === "ready_review" && execution && (
        ...
        >
          <h3>You submit this one.</h3>
          <p>
            Submit on the school page, then tell me the confirmation you see.
          </p>
          <label>
          ...
          <button
            className="button"
            disabled={!confirmation.trim() || busy !== null}
          >
