...
export function schoolScanPresentation(state: SchoolOnboardingState) {
  ...
  const selectedAssignment = state.assignments.find(
    (assignment) => assignment.assignmentId === scan?.targetAssignmentId,
  );
  const running = scan?.state === "running";
  const paused = scan?.state === "needs_user";
  const active = running || paused;
  const signIn =
    paused && (scan.handoff?.kind === "school_sign_in" || scan.handoff?.kind === "linked_system_sign_in");
  const directory = scan?.inventories.find((item) => item.kind === "courses");
  const courses = directory
    ? state.courses.filter((course) => directory.itemIds.includes(course.courseId))
    : state.courses;
  const checkedIds = new Set(
    scan?.inventories.filter((item) => item.kind === "assignments").map((item) => item.courseId),
  );
  const checked = courses.filter((course) => checkedIds.has(course.courseId)).length;
  const checkedLabel = !directory
    ? "Class list not yet verified"
    : `${checked} of ${courses.length} classes checked`;
  ...
  const counts = [
    added ? `${added} new ${added === 1 ? "assignment" : "assignments"}` : "",
  ...
  const description = running
    ...
    : paused
      ? signIn
        ? (scan.handoff?.reason ?? scan.currentStep)
        : `Scan paused · ${checkedLabel.toLowerCase()}`
      : scan?.state === "failed"
        ? (scan.failures[0] ?? "The scan stopped before it finished.")
        : scan
          ? counts ||
            (scan.state === "succeeded"
              ? "No new or changed homework in this scan."
              : "Some school pages still need checking.")
  ...
  return {
    ...
    courses: scan?.targetAssignmentId ? [] : courses,
    checkedIds,
    checkedLabel: scan?.targetAssignmentId ? "Selected assignment only" : checkedLabel,
    title: scan?.targetAssignmentId
      ? running
        ? "Checking this assignment…"
        : paused
          ? title
          : scan.state === "succeeded"
            ? "Assignment checked."
            : "This assignment needs another look."
      : title,
    description: scan?.targetAssignmentId
      ? `${selectedAssignment?.title ?? "Selected assignment"} · ${scan.handoff?.reason ?? scan.currentStep}`
      : description,
    mood: running
      ? ("scanning" as const)
      : paused
        ...
        : scan?.state === "failed"
          ...
          : scan
            ? ("done" as const)
            : ("hello" as const),
    incompleteLabel: gaps.length
      ? `Still needs checking: ${gaps.join(", ")}.`
      : "Some homework sources weren’t fully checked. See Scan details.",
...
export function scanChangeLabel(change: ScanChange): string {
  ...
  const labels = change.fields.map(
    (field) =>
      ({ dueAt: "Due date", dueText: "Due date", instructions: "Instructions", title: "Title" })[field] ??
      "Details",
  );
