...
export function FileMark({ path }: { path: string }) {
  return (
    <span className="assignment-file-mark" aria-hidden="true">
      ...
      <b>{path.includes(".") ? path.split(".").pop()?.slice(0, 4).toUpperCase() : "FILE"}</b>
    </span>
  );
}
...
export function HomeworkFiles({
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const visible = items.filter(
        (item) => item.kind === "file" && !item.path.split("/").some((part) => part.startsWith(".studi-")),
      );
      if (mounted.current) {
        setFiles(visible);
      ...
      if (mounted.current) setListError(cause instanceof Error ? cause.message : String(cause));
    } finally {
  ...
  const reveal = async (path?: string) => {
    ...
    setError("");
    try {
      await window.studi?.openAssignmentFolder({ assignmentId, ...(path ? { path } : {}) });
    } catch (cause) {
      if (mounted.current) setError(cause instanceof Error ? cause.message : String(cause));
    }
  };
  ...
  return (
    <div className="assignment-files" aria-label="Assignment files">
      {opened ? (
        <>
          <div className="assignment-reader-nav">
            <button
            ...
            <button className="assignment-text-action" onClick={() => void reveal(opened.path)}>
              Show in folder ↗
            </button>
          </div>
          ...
          {reading ? (
            ...
            <article className="assignment-document" aria-label="File preview">
              <pre>{content}</pre>
            </article>
          ) : (
            !error && (
        ...
        <>
          <div className="assignment-section-heading">
            <h2>
              Files <span>{loading ? "" : files.length}</span>
            </h2>
            <button className="assignment-text-action" disabled={adding} onClick={() => void add()}>
              {adding ? "Adding files…" : "+ Add files"}
            </button>
          </div>
          {loading ? (
            ...
            (files.length ? (
              files.map((file) => (
                <button key={file.path} className="assignment-file-row" onClick={() => void read(file)}>
              ...
              <div className="assignment-file-empty">
                ...
                <h3>A place for your materials.</h3>
          ...
          <button className="assignment-text-action assignment-folder-link" onClick={() => void reveal()}>
      ...
      {listError && !opened && (
        <div className="assignment-file-error" role="alert">
          <p>{listError}</p>
          <button className="assignment-text-action" onClick={() => void refresh()}>
        ...
        <div className="assignment-file-error" role="alert">
          ...
          {retryOperation && (
            <button
              ...
              onClick={() => void (retryOperation === "read" && opened ? read(opened) : add())}
            >
              Try again
            </button>
          )}
