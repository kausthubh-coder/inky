...
export function ConnectedAppRow({
        ...
        : failed
          ...
          : active
            ? checked
              ...
              : "Connected"
            : waiting
              ? checked && feedback.operation === "check"
                ? "Still waiting — finish signing in."
                : "Finish signing in in your browser."
              : needsReconnect
                ? "Connection expired or unavailable. Sign in again."
                : checked
                  ? "No connection found. Connect again."
                  : "Not connected";
  let label =
    feedback?.phase === "checking"
      ? "Checking…"
  ...
  if (onboarding) {
    ...
    message = pending
      ? feedback.phase === "connecting"
        ? "Connecting… Finish signing in in your browser."
        : "Checking connection…"
      : failed || needsReconnect
        ? "Connection failed."
        : waiting
  ...
  return (
    <div
      className={onboarding ? "fable-pick fable-connected-app" : "connected-app-row"}
      data-connected-app={toolkit}
    >
      ...
      <span className="connected-app-copy">
        ...
        <small>{access}</small>
        <span
          className={`connection-feedback connection-feedback--${tone}`}
          id={statusId}
          role="status"
          aria-live="polite"
          aria-atomic="true"
        >
          <i
            className={pending ? "connection-feedback__spinner" : "connection-feedback__mark"}
            aria-hidden="true"
          >
            {pending ? null : failed || needsReconnect ? "!" : active ? "✓" : waiting ? "…" : "·"}
          </i>
          {message}
        </span>
      </span>
      {!(onboarding && active && !pending && !failed) && (
        <button
          className={onboarding ? "fable-button" : "quiet-button"}
          type="button"
          disabled={disabled || pending}
          aria-describedby={statusId}
          onClick={() => (!onboarding && check ? onCheck(toolkit) : onConnect(toolkit))}
        >
          {label}
        </button>
      )}
