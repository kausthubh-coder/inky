...
export function AssignmentWorkspace({
  ...
  const course =
    onboarding.courses.find((course) => course.courseId === assignment.courseId)?.label ??
    assignment.courseId;
  ...
  const due = assignment.dueAt
    ...
    : (assignment.dueText ?? "No due date listed");
  ...
  return (
    <section
      className={`assignment-workspace course-accent-${courseTone(course)}`}
    ...
    >
      <header className="assignment-heading">
        ...
        <button className="chat-icon assignment-close" aria-label="Close assignment" onClick={onClose}>
          <Icon name="close" />
        </button>
        <div className="assignment-meta">
          <span className="assignment-due">
            <Icon name="calendar" size={14} />
            {assignment.dueAt || assignment.dueText ? "Due " : ""}
            <time dateTime={assignment.dueAt}>{due}</time>
          </span>
          <span className={`assignment-progress progress-${status.tone}`} role="status">
            <span aria-hidden="true">{state === "submitted" ? "✓" : "—"}</span>
            {status.label}
          </span>
          <button className="assignment-text-action" onClick={onBrowser} aria-expanded={Boolean(browser)}>
            Open school page ↗
          </button>
        </div>
      </header>
      <div className="assignment-body">
        <div className="assignment-page">
          <div className="assignment-tabs" role="tablist" aria-label="Assignment sections">
            {tabs.map(({ id, label }, index) => (
              <button
                ...
                aria-controls={`assignment-panel-${id}`}
              ...
              >
                {label}
                {id === "files" && fileCount !== null && <span>{fileCount}</span>}
              </button>
            ))}
          </div>
          {browser && <div className="assignment-school">{browser}</div>}
          <section
            className="assignment-page-scroll assignment-brief"
            role="tabpanel"
            id="assignment-panel-assignment"
            aria-labelledby="assignment-tab-assignment"
            hidden={Boolean(browser) || tab !== "assignment"}
            tabIndex={0}
          >
            ...
            {assignment.requirementEvidence?.length ? (
              ...
              <p>
                {assignment.instructions ??
                  "No instructions saved yet. Open the school page for the full requirements."}
              </p>
            )}
            {assignment.missingRequirements?.length ? (
              <div role="status">
                ...
                <ul>
                  {assignment.missingRequirements.map((item, index) => (
                    <li key={index}>{item}</li>
                  ))}
                </ul>
              </div>
            ) : null}
            <button className="assignment-material-link" onClick={() => selectTab("files")}>
              <Icon name="note" />
              Files for this assignment{fileCount !== null && <span>{fileCount}</span>}
              <Icon name="right" />
            </button>
          </section>
          <section
            className="assignment-page-scroll"
            role="tabpanel"
            id="assignment-panel-files"
            aria-labelledby="assignment-tab-files"
            hidden={Boolean(browser) || tab !== "files"}
            tabIndex={0}
          >
            <HomeworkFiles
          ...
          <section
            className="assignment-page-scroll"
            role="tabpanel"
            id="assignment-panel-work"
            aria-labelledby="assignment-tab-work"
            hidden={Boolean(browser) || tab !== "work"}
            tabIndex={0}
          >
            <AssignmentWork
              execution={execution}
        ...
        <aside className="assignment-inky" aria-label="Inky">
          <div className="assignment-inky-scroll">
            <AssignmentSummary
              ...
              onResume={onResume}
              onPause={onPause}
              onBrowser={state === "ready_review" ? () => selectTab("work") : onBrowser}
              onAnswer={() => selectTab("work")}
            ...
            {execution && ["working", "needs_user", "ready_review"].includes(execution.phase) && (
              <button
                className="assignment-text-action assignment-stop"
                disabled={busy !== null && !(busy === "assignment" && execution.phase === "working")}
