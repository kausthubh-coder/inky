...
export class RendererTelemetry {
  ...
  constructor(
    readonly loadModule: () => Promise<PostHogModule> = () =>
      import("posthog-js/dist/module.full.no-external.js"),
  ) {}
  ...
  #sendContext(sessionId: string, windowId: string): void {
    ...
    this.#contextKey = key;
    void window.studi
      .captureUiTelemetry({ event: "replay_context", distinctId, sessionId, windowId })
      .then((accepted) => {
        if (!accepted && this.#contextKey === key) this.#contextKey = null;
      })
      .catch(() => {
        if (this.#contextKey === key) this.#contextKey = null;
      });
  }
        ...
        client.init(state.rendererConfig!.projectToken, {
          ...
          session_recording: {
            ...
            maskInputFn: (text, element) =>
              element?.getAttribute("type")?.toLowerCase() === "password" || element?.closest("[data-secret]")
                ? "[secret]"
                : stripSecrets(text),
