    ...
    <main className="preview-gallery" data-studi-preview-gallery="true">
      <header className="preview-gallery__hero">
        <div>
          ...
          <p>
            These are the real components with controlled local data. Open one to inspect it at full size; no
            Electron, Clerk, Convex, or school login is required.
          </p>
          <p className="preview-source">
            ...
            <br />
            <small>{source?.root ?? "Run bun run preview:ui for checkout details."}</small>
          </p>
        </div>
        <a className="button button--yellow" href="/?preview=week">
          Open the week
        </a>
      </header>
      {groups.map((group) => (
        <section className="preview-gallery__group" key={group}>
          <div className="preview-gallery__heading">
            <h2>{group}</h2>
            <span>{DEV_PREVIEW_SCENARIOS.filter((scenario) => scenario.group === group).length} views</span>
          </div>
          <div className="preview-gallery__grid">
            {DEV_PREVIEW_SCENARIOS.filter((scenario) => scenario.group === group).map((scenario) => (
              <article className="preview-card" key={scenario.id}>
                ...
                >
                  <iframe
                    src={`/?preview=${scenario.id}&frame=1`}
