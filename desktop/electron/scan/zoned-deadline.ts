...
export function parseZonedDeadline(text: string): number | null {
  ...
  if (!zone) return null;
  const dateText = text.slice(0, zone.index).trim().replace(/\s+at\s+/i, " ");
  const date = dateText.match(/^(?:[A-Za-z]+,\s*)?([A-Za-z]+)\s+(\d{1,2}),?\s+(\d{4})\s+(\d{1,2}):(\d{2})(?::(\d{2}))?\s*(AM|PM)?$/i);
  if (!date) return NaN;
  ...
  if (month < 0 || year < 100 || day < 1 || day > 31 || minute > 59 || second > 59
    || (meridiem ? hour < 1 || hour > 12 : hour > 23)) return NaN;
  ...
  if (calendar.getUTCFullYear() !== year || calendar.getUTCMonth() !== month || calendar.getUTCDate() !== day) return NaN;
  const weekday = dateText.match(/^([A-Za-z]+),/)?.[1]?.toLowerCase();
  if (weekday && ![weekdays[calendar.getUTCDay()], weekdays[calendar.getUTCDay()]!.slice(0, 3)].includes(weekday)) return NaN;

  let formatter: Intl.DateTimeFormat;
  try {
    formatter = new Intl.DateTimeFormat("en-US", { timeZone: zone[1], calendar: "gregory", numberingSystem: "latn",
  ...
  const candidates = [...offsets].map(offset => wallTime - offset).filter(instant => observedWallTime(instant) === wallTime);
