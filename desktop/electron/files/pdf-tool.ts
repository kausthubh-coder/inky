...
export function createPdfReadTool(files: HomeworkFiles) {
  return defineTool({
    ...
    description:
      "Read one page of a PDF in this assignment's workspace. Returns extracted text, total page count, and a page image by default so you can read scans, diagrams, and equations. Start at page 1 and continue through the required pages. Set image=false only when the text alone is sufficient.",
    ...
    execute: async (_id, input, signal) => {
      ...
      if (!Buffer.from(data.subarray(0, 1024)).includes(Buffer.from("%PDF-"))) {
        throw new Error(
          "This file is not a PDF. Inspect the school link; it may have returned a login page instead.",
        );
      }
      ...
      try {
        ...
        if (input.image !== false) {
          ...
          try {
            ...
            content.push({
              type: "image",
              mimeType: "image/png",
              data: (await canvas.encode("png")).toString("base64"),
            });
          } finally {
        ...
        return {
          ...
          details: {
            ...
            textTruncated: text.length > 20_000,
          },
        };
      } finally {
