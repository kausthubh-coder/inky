...
import type {
  ...
  ConnectedAppExecution,
  ConnectedAppsState,
  ConnectedAppToolSearch,
  FeedbackReceipt,
  UsageRecordInput,
  UsageState,
...
const bootstrapAccount = makeFunctionReference<"mutation", { deviceId: string }, CloudAccountResult>(
  "account:bootstrap",
);
const releaseAccountDevice = makeFunctionReference<"mutation", { deviceId: string }, null>(
  "account:releaseDevice",
);
const submitAccountFeedback = makeFunctionReference<
  "mutation",
  { deviceId: string; feedbackId: string; message: string },
  FeedbackReceipt
>("feedback:submit");
const getConnectedApps = makeFunctionReference<"action", Record<string, never>, ConnectedAppsState>(
  "composio:status",
);
const authorizeConnectedApp = makeFunctionReference<"action", { toolkit: string }, ConnectedAppConnection>(
  "composio:authorize",
);
const getConnectedAppConnection = makeFunctionReference<
  "action",
  { toolkit: string },
  ConnectedAppConnection
>("composio:connection");
const searchConnectedAppTools = makeFunctionReference<
  "action",
  { toolkit: string; query: string },
  ConnectedAppToolSearch
>("composio:search");
const executeConnectedAppTool = makeFunctionReference<
  "action",
  { toolkit: string; toolSlug: string; arguments: Record<string, unknown> },
  ConnectedAppExecution
>("composio:execute");
const getCurrentUsage = makeFunctionReference<"query", { period: string; throughDate: string }, UsageState>(
  "usage:current",
);
