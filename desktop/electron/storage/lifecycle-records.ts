...
function parseScheduleRow(row: ScheduleRow): AutomationSchedule {
  ...
  assertColumns("automation schedule", record.scheduleId, row, {
...
function parseExecutionRow(row: ExecutionRow): AssignmentExecution {
  ...
  assertColumns("assignment execution", record.taskId, row, {
    task_id: record.taskId,
    assignment_id: record.assignmentId,
    phase: record.phase,
    review_deadline: record.reviewDeadline ?? null,
    updated_at: record.updatedAt,
  });
  return record;
}
...
function parseAttemptRow(row: AttemptRow): ExecutionAttempt {
  ...
  assertColumns("execution attempt", `${record.taskId}/${record.ordinal}`, row, {
    task_id: record.taskId,
...
function parseNotificationRow(row: NotificationRow): NotificationIntent {
  ...
  assertColumns("notification intent", record.notificationId, row, {
...
function parseReceiptRow(row: ReceiptRow): SubmissionReceipt {
  ...
  assertColumns("submission receipt", record.receiptId, row, {
...
export class LifecycleRepository {
  ...
  putSchedule(value: unknown): AutomationSchedule {
    ...
    `)
      .run(
        ...
        schedule.updatedAt,
        json(AutomationScheduleSchema, schedule),
      );
    return schedule;
  }
  ...
  getSchedule(): AutomationSchedule | null {
    const row = this.database.handle
      .prepare(
        "SELECT schedule_id, state, cadence, next_run_at, updated_at, record_json FROM automation_schedules WHERE schedule_id = 'school-scan'",
      )
  ...
  claimDueSchedule(now: string, nextRunAt: string): AutomationSchedule | null {
    return this.database.transaction(() => {
      ...
      if (!current || current.state !== "enabled" || !current.nextRunAt || current.nextRunAt > now)
        return null;
      const claimed = current.nextRunAt;
      const next = this.putSchedule({
        ...current,
        nextRunAt,
        lastClaimedOccurrence: claimed,
        updatedAt: now,
      });
  ...
  putExecution(value: unknown): AssignmentExecution {
    ...
    `)
      .run(
        ...
        execution.phase,
        execution.reviewDeadline ?? null,
        execution.updatedAt,
        json(AssignmentExecutionSchema, execution),
      );
    return execution;
  }
  ...
  getExecution(taskId: string): AssignmentExecution | null {
    const row = this.database.handle
      .prepare(
        "SELECT task_id, assignment_id, phase, review_deadline, updated_at, record_json FROM assignment_executions WHERE task_id = ?",
      )
  ...
  latestExecution(): AssignmentExecution | null {
    const row = this.database.handle
      .prepare(
        "SELECT task_id, assignment_id, phase, review_deadline, updated_at, record_json FROM assignment_executions ORDER BY updated_at DESC, task_id LIMIT 1",
      )
      .get() as ExecutionRow | undefined;
    return row ? parseExecutionRow(row) : null;
  }
  ...
  addAttempt(value: unknown): ExecutionAttempt {
    ...
    if (prior.length >= 2)
      throw new StorageError(
        "record_validation_failed",
        "An assignment may record at most two recovery attempts",
        { taskId: attempt.taskId },
      );
    if (prior.some((item) => item.plan.trim().toLowerCase() === attempt.plan.trim().toLowerCase())) {
      throw new StorageError("record_validation_failed", "Recovery plans must be meaningfully different", {
        taskId: attempt.taskId,
      });
    }
    this.database.handle
  ...
  listAttempts(taskId: string): ExecutionAttempt[] {
    const rows = this.database.handle
      .prepare(
        "SELECT task_id, ordinal, recorded_at, record_json FROM execution_attempts WHERE task_id = ? ORDER BY ordinal",
      )
      .all(taskId) as unknown as AttemptRow[];
  ...
  putNotification(value: unknown): NotificationIntent {
    ...
    `)
      .run(
        ...
        intent.clickedAt ?? null,
        json(NotificationIntentSchema, intent),
      );
    return intent;
  }
  ...
  getNotification(notificationId: string): NotificationIntent | null {
    const row = this.database.handle
      .prepare(
        "SELECT notification_id, kind, target_type, target_id, created_at, delivered_at, clicked_at, record_json FROM notification_intents WHERE notification_id = ?",
      )
  ...
  listNotifications(): NotificationIntent[] {
    const rows = this.database.handle
      .prepare(
        "SELECT notification_id, kind, target_type, target_id, created_at, delivered_at, clicked_at, record_json FROM notification_intents ORDER BY created_at DESC, notification_id DESC LIMIT 100",
      )
  ...
  latestNotification(): NotificationIntent | null {
    const row = this.database.handle
      .prepare(
        "SELECT notification_id, kind, target_type, target_id, created_at, delivered_at, clicked_at, record_json FROM notification_intents ORDER BY created_at DESC, notification_id DESC LIMIT 1",
      )
  ...
  getSubmissionReceipt(taskId: string): SubmissionReceipt | null {
    const row = this.database.handle
      .prepare(
        "SELECT receipt_id, task_id, submitted_at, record_json FROM submission_receipts WHERE task_id = ?",
      )
      .get(taskId) as ReceiptRow | undefined;
...
export function validateLifecycleRecords(database: StudiSqliteDatabase): void {
  ...
  const tasks = database.handle
    .prepare("SELECT task_id FROM assignment_executions")
