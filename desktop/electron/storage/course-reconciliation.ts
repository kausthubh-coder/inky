...
export function courseIdentity(store: LocalStore, course: Course): string | null {
  const targets = [
    course.sourceTarget,
    course.evidence.sourceTarget,
    ...store.assignments
      .listByCourse(course.courseId)
      .flatMap((assignment) => [
        assignment.sourceTarget,
        ...assignment.evidence.map((evidence) => evidence.sourceTarget),
      ]),
  ];
...
export function reconcileCourses(store: LocalStore): CourseConflict[] {
  return store.database.transaction(() => {
    ...
    for (const group of groups.values()) {
      ...
      const modes = new Set(
        group.map((course) => {
          const resolution = resolvePermission(
            {
              assignmentId: "course-reconciliation",
      ...
      const patterns = group.map((course) =>
        JSON.stringify(
          rules
            .filter((rule) => rule.scope === "pattern" && rule.courseId === course.courseId)
            .map((rule) => [rule.scope === "pattern" ? rule.patternId : "", rule.mode, rule.updatedAt])
            .sort(),
        ),
      );
      ...
      if (modes.size > 1 || new Set(patterns).size > 1 || protectedCourses.length > 1) {
        conflicts.push({
          courseIds: group.map((course) => course.courseId),
          kind: modes.size > 1 || new Set(patterns).size > 1 ? "permissions" : "references",
          reason:
            modes.size > 1 || new Set(patterns).size > 1
      ...
      const keeper =
        ...
        [...group].sort(
          (a, b) =>
            store.assignments.listByCourse(b.courseId).length -
              store.assignments.listByCourse(a.courseId).length ||
      ...
      for (const donor of group) {
        ...
        store.database.handle
          .prepare(
            "INSERT INTO record_redirects(kind, old_id, canonical_id, record_json) VALUES ('course', ?, ?, ?)",
          )
        ...
        for (const table of [
          ...
          store.database.handle
            .prepare(
              `UPDATE ${table} SET course_id = ?, record_json = json_set(record_json, '$.courseId', ?) WHERE course_id = ?`,
            )
...
function hasExternalReference(store: LocalStore, id: string): boolean {
  ...
  const contains = (directory: string): boolean =>
  ...
  for (const row of store.database.handle
    ...
    if (
      store.database.handle
        ...
        .all()
        .some((record) => String(record.record_json).includes(JSON.stringify(id)))
    )
      return true;
  }
  return false;
