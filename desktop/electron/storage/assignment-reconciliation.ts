  ...
  return store.database.transaction(() => {
    ...
    for (const [identity, group] of groups) {
      ...
      const protectedRecords = records.filter((record) =>
        hasWork(store, record.assignment, record.tasks, files),
      );
      const modes = new Set(
        group.map(
          (assignment) =>
            resolvePermission(
              {
                ...
                matchedPatternIds: store.manager
                  .listConfirmedPatterns(assignment.assignmentId, assignment.courseId)
                  .map((match) => match.patternId),
              },
              store.permissionRules.listAll(),
            ).mode,
        ),
      );
      const orphanedWork =
        protectedRecords.length === 1 &&
        protectedRecords[0]!.tasks.length === 0 &&
        records.some((record) => record.tasks.length > 0);
      const evidenceConflict = group.some((left, index) =>
        group.slice(index + 1).some((right) => conflictingSchoolFacts(left, right)),
      );
      if (
        evidenceConflict ||
        protectedRecords.length > 1 ||
        modes.size > 1 ||
        orphanedWork ||
        records.some((record) => record.tasks.length > 1)
      ) {
        conflicts.push({
          assignmentIds: group.map((item) => item.assignmentId),
      ...
      const keeper =
        ...
        records.sort(
          (a, b) =>
            ...
            a.assignment.discoveredAt.localeCompare(b.assignment.discoveredAt) ||
            a.assignment.assignmentId.localeCompare(b.assignment.assignmentId),
        )[0]!;
      ...
      for (const donor of records) {
        ...
        for (const task of donor.tasks) {
          ...
          const queue = store.database.handle
            .prepare("SELECT record_json FROM manager_queue WHERE task_id = ?")
            .get(task.taskId);
          archive(store, "task", task.taskId, canonicalTask.taskId, {
            task,
            events: store.tasks.listEvents(task.taskId),
            queue: queue ?? null,
          });
        ...
        kept = {
          ...
          evidence: [
            ...new Map(
              [...donor.assignment.evidence, ...kept.evidence].map((item) => [item.evidenceId, item]),
            ).values(),
          ],
        };
...
function archive(
  ...
  store.database.handle
    .prepare("INSERT INTO record_redirects(kind, old_id, canonical_id, record_json) VALUES (?, ?, ?, ?)")
...
function hasWork(store: LocalStore, assignment: Assignment, tasks: Task[], files: string[]): boolean {
  if (tasks.some((task) => store.manager.getQueueEntry(task.taskId)?.requestOrigin === "student"))
    return true;
  if (
    tasks.some(
      (task) =>
        !["discovered", "queued"].includes(task.state) ||
        store.tasks
          .listEvents(task.taskId)
          .some(
            (event) =>
              event.type !== "task_created" &&
              !(event.payload.from === "discovered" && event.payload.to === "queued"),
          ),
    )
  )
    return true;
...
function mergeSchoolFacts(kept: Assignment, donor: Assignment): Partial<Assignment> {
  const newer = <T extends { evidence: { capturedAt: string } }>(
    left: T | undefined,
    right: T | undefined,
  ): T | undefined =>
    !left ? right : right && right.evidence.capturedAt > left.evidence.capturedAt ? right : left;
  const deadline =
    !kept.deadlineEvidence && !donor.deadlineEvidence
      ? {
          dueAt: kept.dueAt ?? donor.dueAt,
          dueText: kept.dueText ?? donor.dueText,
          deadlinePrecision: kept.deadlinePrecision ?? donor.deadlinePrecision,
          deadlineEvidence: undefined,
        }
      : !kept.deadlineEvidence ||
          (donor.deadlineEvidence && donor.deadlineEvidence.capturedAt > kept.deadlineEvidence.capturedAt)
        ? donor
        : kept;
  const requirements = [
    ...new Map(
      [...(donor.requirementEvidence ?? []), ...(kept.requirementEvidence ?? [])].map((item) => [
        item.text,
        item,
      ]),
    ).values(),
  ];
  const differentRequirements = Boolean(
    kept.requirementEvidence?.length &&
      donor.requirementEvidence?.length &&
      JSON.stringify(kept.requirementEvidence.map((item) => item.text).sort()) !==
        JSON.stringify(donor.requirementEvidence.map((item) => item.text).sort()),
  );
  const missing = [
    ...new Set([
      ...(kept.missingRequirements ?? []),
      ...(donor.missingRequirements ?? []),
      ...(differentRequirements
        ? ["Reconcile the different requirement excerpts recorded for these source aliases."]
        : []),
    ]),
  ];
  return {
    schoolStatus: newer(kept.schoolStatus, donor.schoolStatus),
    latePolicy: newer(kept.latePolicy, donor.latePolicy),
    dueAt: deadline.dueAt,
    dueText: deadline.dueText,
    deadlinePrecision: deadline.deadlinePrecision,
    deadlineEvidence: deadline.deadlineEvidence,
    requirementEvidence: requirements,
    requirementsState:
      !missing.length && (kept.requirementsState === "complete" || donor.requirementsState === "complete")
        ? "complete"
        : "partial",
...
function conflictingSchoolFacts(left: Assignment, right: Assignment): boolean {
  const sameTime = (a: { capturedAt: string } | undefined, b: { capturedAt: string } | undefined) =>
    Boolean(a && b && a.capturedAt === b.capturedAt);
  return (
    (sameTime(left.schoolStatus?.evidence, right.schoolStatus?.evidence) &&
      left.schoolStatus?.state !== right.schoolStatus?.state) ||
    (sameTime(left.latePolicy?.evidence, right.latePolicy?.evidence) &&
      (left.latePolicy?.state !== right.latePolicy?.state ||
        left.latePolicy?.until !== right.latePolicy?.until)) ||
    (sameTime(left.deadlineEvidence, right.deadlineEvidence) &&
      (left.dueAt !== right.dueAt ||
        left.deadlinePrecision !== right.deadlinePrecision ||
        (!left.dueAt && left.dueText !== right.dueText)))
  );
}
