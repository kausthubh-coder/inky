...
export default function UsagePage() {
  ...
  const period = new Date(`${usage.period}-01T00:00:00Z`).toLocaleDateString("en-US", {
    month: "long",
    year: "numeric",
    timeZone: "UTC",
  });
  const empty = usage.assignments === 0 && usage.browserMinutes === 0 && usage.agentTokens === 0;
  ...
  return (
    <>
      ...
      <section className={`account-panel ${styles.sheet}`} aria-label="Monthly usage">
        ...
        <div className={styles.credits}>
          ...
          <strong>{credits === null ? "Available when your access opens" : credits.toLocaleString()}</strong>
        </div>
      </section>
    </>
  );
