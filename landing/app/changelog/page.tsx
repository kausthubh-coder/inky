...
export const metadata: Metadata = {
  ...
  description:
    "Every Studi release, from the first hello to the latest improvements. See what’s new with Inky.",
};
    ...
    <>
      ...
      <main className={styles.page} id="main">
        <header className={styles.intro}>
          ...
          <a className="btn primary" href={releaseUrl(changelog[0].version)}>
            Get the latest version <span aria-hidden="true">↗</span>
          </a>
          ...
          <div className={styles.releases}>
            {changelog.map((release, index) => (
              <article
                ...
                key={release.version}
                aria-labelledby={`title-${release.version}`}
              >
                <div className={styles.meta}>
                  ...
                  <time dateTime={release.date}>
                    {dateFormat.format(new Date(`${release.date}T12:00:00Z`))}
                  </time>
                </div>
                ...
                <a className={styles.releaseLink} href={releaseUrl(release.version)}>
                  Release notes & downloads <span aria-hidden="true">↗</span>
                  <span className="visually-hidden"> for v{release.version}</span>
                </a>
              </article>
            ))}
          </div>
