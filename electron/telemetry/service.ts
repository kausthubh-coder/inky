...
export const telemetryEventSchemas = {
  ...
  studi_scan_started: z.strictObject({ mode: z.enum(["start", "resume", "replay", "scheduled"]) }),
...
export class TelemetryService {
  ...
  capture<Name extends TelemetryEventName>(event: Name, input: EventProperties<Name>): boolean {
    ...
    const properties = telemetryEventSchemas[eventName].parse(input) as Record<string, string | number | boolean>;
    this.#expireDebug();
  ...
  identifyClerk(subject: string): void {
    this.#distinctId = z.string().min(1).max(256).regex(/^[A-Za-z0-9._:-]+$/).parse(subject);
    this.#identity = "clerk";
  }
...
function sanitizeSdkMessage(message: unknown) {
  ...
  if (!eventResult.success || !distinctIdResult.success) return null;
  const properties = record.properties;
  if (!properties || typeof properties !== "object" || Array.isArray(properties)) return null;
  const propertyRecord = properties as Record<string, unknown>;
  const schema = telemetryEventSchemas[eventResult.data];
  const eventProperties = schema.safeParse(pickSchemaProperties(eventResult.data, propertyRecord));
