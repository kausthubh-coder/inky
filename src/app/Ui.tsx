...
export function TelemetryControls({
  ...
  return (
    <PaperCard tone="lavender" className="settings-card">
      ...
      <label className="toggle-row"><input type="checkbox" checked={telemetry?.enabled ?? false} disabled={!configured || busy} onChange={(event) => onChange(event.target.checked, telemetry?.replayEnabled ?? true)} /><span><strong>Share product events</strong><small>Private beta already includes this. We keep school passwords, cookies, and answers off PostHog.</small></span></label>
