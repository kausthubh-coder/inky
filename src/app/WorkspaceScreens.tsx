...
import { AppChrome, type AppScreen, Field, PaperCard, StatusPill, TelemetryControls, executionLabel, formatDateTime, formatDue } from "./Ui.js";
...
export function DashboardScreen({
  ...
  const inkyState: InkyState = scan?.state === "running" ? "scanning" : scan?.state === "partial" || scan?.state === "failed" || scan?.state === "needs_user" ? "needs" : scan?.state === "succeeded" ? "done" : "idle";
  ...
  return (
    <main className="app-shell" data-studi-app-ready="true">
      ...
      <div className="page dashboard-page">
        ...
        {scan?.state === "partial" && <div className="truth-banner truth-banner--partial"><strong>This view is partial.</strong><span>{scan.failures[0] ?? "Some school coverage could not be verified."}</span><button onClick={onScanAgain}>Retry visible scan</button></div>}
        {scan?.state === "failed" && <div className="truth-banner truth-banner--error"><strong>The latest scan failed.</strong><span>Prior verified assignments remain; nothing new is marked complete.</span><button onClick={onScanAgain}>Retry</button></div>}
