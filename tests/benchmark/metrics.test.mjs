...
const run = () => ({ schemaVersion: 1, runId: "test-run", evidenceClass: "live-production-runtime",
  fixture: { scenarioId: "school", version: 1, seed: 0, clock: "2026-09-14T12:00:00Z", contentHash: "content-a" },
  config: { model: "test-model", provider: "test-provider", effort: "medium", budgetMs: 1000, maxToolCalls: 20, phases: ["cold", "unchanged"] },
...
test("aggregate includes every phase and failure without changing its input", () => {
  ...
  assert.deepEqual(aggregateMetrics(phases), { durationMs: 101, toolCalls: 8, modelCalls: 4,
...
test("empty and missing measurements stay unknown, including genuine partial usage", () => {
  assert.deepEqual(aggregateMetrics([]), { durationMs: null, toolCalls: null, modelCalls: null, usage: null });
  ...
  assert.deepEqual(aggregateMetrics(phases).usage, { inputTokens: 20, outputTokens: 10, cacheReadTokens: null, cacheWriteTokens: 0 });
});
...
test("invalid numbers cannot become zero or corrupt totals", () => {
  ...
  assert.deepEqual(aggregateMetrics([{ metrics: { durationMs: 0, toolCalls: 0, modelCalls: 0 } }]),
    { durationMs: 0, toolCalls: 0, modelCalls: 0, usage: null });
});
...
test("paired comparison permits candidate revision changes and returns signed deltas", () => {
  ...
  assert.deepEqual(result.deltas, { durationMs: -30, toolCalls: -2, modelCalls: 0,
...
test("each changed fixture or configuration field refuses comparison", () => {
  for (const group of ["fixture", "config"]) {
    for (const field of Object.keys(run()[group])) {
      const after = run(), previous = after[group][field];
      after[group][field] = Array.isArray(previous) ? [...previous, "resumed"]
        : typeof previous === "number" ? previous + 1 : `${previous}-changed`;
...
test("missing equal metadata, unknown settings and exploratory evidence cannot silently compare", () => {
  for (const mutate of [
    ...
    r => { r.config.viewport = { width: 1200 }; },
    r => { r.evidenceClass = "external-codex-exploratory"; },
    r => { r.schemaVersion = 2; },
    r => delete r.revision.buildTreeSha256,
  ]) {
    const before = run(), after = run(); mutate(before); mutate(after);
    assert.equal(compareRuns(before, after).comparable, false);
  }
  const after = run(); after.evidenceClass = "controlled-production-replay";
...
test("failed and incomplete attempts remain measured and cannot pass via a forged summary", () => {
  for (const mutate of [
    ...
    r => { r.phases[1].grade.checks = []; },
    r => { r.phases[1].grade.checks[0].passed = "true"; },
    r => { r.phases[1].metrics.toolCalls = 18; },
    r => { r.phases[1].metrics.durationMs = 999; },
  ]) {
    const candidate = run(); mutate(candidate);
...
test("replay can explicitly have no model; required phases cannot be empty or duplicated", () => {
  const replay = run(); replay.evidenceClass = "controlled-production-replay";
