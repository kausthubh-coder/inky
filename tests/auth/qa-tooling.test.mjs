...
import {
  ...
  summarize,
  invitationUrl,
} from "../../.agents/skills/test-studi/scripts/qa-account.mjs";
...
test("QA account lookup rejects personal identities and filters fuzzy paginated matches", async () => {
  ...
  const state = await lookup(async (path) => {
    ...
    if (path.startsWith("/waitlist"))
      return { data: [{ email_address: "wrong@example.com" }], total_count: 1 };
    if (path.includes("offset=0"))
      return Array.from({ length: 100 }, () => ({ email_address: "wrong@example.com" }));
...
test("account setup links an invitation before optionally creating a missing user", async () => {
  ...
  await prepare(
    ...
    { email, user: null, entries: [], invitations: [] },
    true,
  );
  assert.deepEqual(
    calls.map((call) => call.path),
    ["/waitlist_entries", "/waitlist_entries/wle_fixture/invite", "/users"],
  );
...
test("existing users are preserved and a missing invitation link stays a failure", async () => {
  const state = {
    email: "studi.qa.fixture+clerk_test@example.com",
    user: { id: "existing" },
    entries: [{ id: "wle", status: "completed", invitation: null }],
  ...
  await assert.rejects(
    prepare(
      async (path, body) => {
        writes.push(path);
        return { status: "completed", invitation: null };
      },
      state,
      true,
    ),
    /no usable linked invitation/,
  );
  ...
  await assert.rejects(
    ...
    /locked, banned, or rejected/,
  );
  assert.throws(
    () =>
      invitationUrl(
        { entries: [], invitations: [{ status: "pending", url: "https://attacker.example/ticket" }] },
        "qa.clerk.accounts.dev",
      ),
    /Unexpected/,
  );
});
...
test("local school fixture saves drafts, escapes answers, and isolates simultaneous schools", async () => {
  const a = await schoolFixture(),
    b = await schoolFixture();
  try {
    assert.notEqual(a.url, b.url);
    const html = await (await fetch(`${a.url}/assignments/observation`)).text();
    const csrf = /name="csrf" value="([^"]+)"/.exec(html)[1];
    const response = await fetch(`${a.url}/assignments/observation`, {
      method: "POST",
      body: new URLSearchParams({ csrf, revision: "0", answer: "<script>unsafe</script>", action: "save" }),
    });
...
test("auth import preserves refreshed profile credentials instead of overwriting with stale cache", async () => {
  ...
  const result = spawnSync(
    process.execPath,
    [
      resolve(".agents/skills/test-studi/scripts/sync-studi-qa-codex-auth.mjs"),
      "--import",
      "--profile",
      profile,
      "--cache",
    ...
    { encoding: "utf8", env: { ...process.env, STUDI_QA_CODEX_AUTH: "invalid-cache" } },
  );
...
test("Windows dry-run with reset preserves the existing profile and chooses distinct ports", {
  skip: process.platform !== "win32" || !existsSync("dist/electron/main.js"),
}, async () => {
  ...
  const result = spawnSync(
    ...
    [
      ...
      "Bypass",
      "-File",
      resolve(".agents/skills/test-studi/scripts/Start-StudiQa.ps1"),
      "-Persistent",
      "-ProfileName",
      name,
      "-ResetPersistent",
  ...
  assert.equal(
    new Set([receipt.cdpEndpoint, receipt.mainInspectorEndpoint, receipt.clerkClaimUrl.replace("/claim", "")])
      .size,
    3,
  );
...
test("Clerk handoff is one-shot per attempt, supports retries, and rejects non-loopback callbacks", async () => {
  const child = spawn(
    process.execPath,
    [
      resolve(".agents/skills/test-studi/scripts/clerk-qa-handoff.mjs"),
      "--port",
      "0",
      "--clerk-host",
      "qa.clerk.accounts.dev",
