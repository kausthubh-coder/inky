  ...
  try {
    ...
    assert.ok(
      runtime
        ...
        .some((model) => model.id === "claude-fable-5-1" && model.providerId === "anthropic"),
    );
    ...
    assert.deepEqual(
      diagnosticRequests.map((event) => event.payload.request),
      requests,
    );
    assert.notEqual(diagnosticRequests[0].run_id, diagnosticRequests[1].run_id);
    assert.ok(
      diagnostics.some(
        (event) => event.kind === "session_created" && event.payload.system_prompt.includes("Studi"),
      ),
    );
    assert.ok(
      diagnostics.some(
        (event) =>
          event.kind === "message_end" && JSON.stringify(event.payload).includes("Check request defaults."),
      ),
    );
