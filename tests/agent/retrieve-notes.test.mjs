...
const note = (noteId, scope, subjectId, about, key) => ({
  ...base,
  noteId,
  scope,
  subjectId,
...
test("assignment retrieval is deterministic and does not leak unrelated courses or unconfirmed patterns", () => {
  ...
  const context = {
    ...
    courseId: "csc-316",
    confirmedPatternIds: ["webassign-weekly"],
    courseAssignmentIds: ["assignment-2", "assignment-3"],
  };
  assert.deepEqual(
    retrieveNoteIndex(entries, context, "automatic").map((entry) => entry.noteId),
    ["note-pref", "note-c", "note-p", "note-self"],
  );
  assert.deepEqual(
    retrieveNoteIndex(entries, context, "search").map((entry) => entry.noteId),
    ["note-pref", "note-c", "note-p", "note-a", "note-self"],
  );
});
...
test("scan retrieval accepts only evidence-independent scan hints for its school", () => {
  ...
  assert.deepEqual(
    retrieveNoteIndex(entries, { kind: "scan", schoolId: "primary-school" }).map((entry) => entry.noteId),
    ["right"],
  );
});
