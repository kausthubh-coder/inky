...
test("capabilities follow target, explicit work, claim, and submit facts", async () => {
  const home = await buildAgentTurn(
    { target: { kind: "home" }, phase: "conversing", hasBrowserClaim: false },
    "hello",
  );
  assert.deepEqual(home.toolNames, [
    "home_status",
    "queue_inspect",
    "queue_start",
    "queue_cancel",
  ...
  const connectedHome = await buildAgentTurn(
    {
      target: { kind: "home" },
      phase: "conversing",
      hasBrowserClaim: false,
      composioTools: ["connected_apps_search", "connected_apps_execute"],
    },
    "email my professor",
  );
  assert.deepEqual(connectedHome.toolNames.slice(-2), ["connected_apps_search", "connected_apps_execute"]);

  const talk = await buildAgentTurn(
    { target: { kind: "assignment", assignmentId: "a-1" }, phase: "conversing", hasBrowserClaim: false },
    "due?",
  );
  assert.deepEqual(talk.toolNames, ["assignment_read", "assignment_start", "note_search", "note_read"]);
  const connectedTalk = await buildAgentTurn(
    {
  ...
  const work = await buildAgentTurn(
    { target: { kind: "assignment", assignmentId: "a-1" }, phase: "working", hasBrowserClaim: true },
    "work",
  );
  assert.equal(work.toolNames.includes("browser_snapshot"), true);
  assert.equal(work.toolNames.includes("browser_submit"), false);

  const submit = await buildAgentTurn(
    {
      target: { kind: "assignment", assignmentId: "a-1" },
      phase: "working",
