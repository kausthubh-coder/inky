...
test("connected apps expose lazy search and execute tools instead of flooding the prompt", async () => {
  ...
  const gateway = {
    connectedApps: async () => ({
      configured: true,
      toolkits: [{ toolkit: "github" }, { toolkit: "gmail" }],
    }),
    connectedAppConnection: async (toolkit) => ({
      toolkit,
      sessionId: "session-1",
  ...
  const tools = await createConnectedAppTools(gateway, {
    observeExecution: (event) => observations.push(event),
  });
  assert.deepEqual(
    tools.map((tool) => tool.name),
    ["connected_apps_search", "connected_apps_execute"],
  );
  assert.deepEqual(tools[0].parameters.properties.toolkit.enum, ["github", "gmail"]);
  const search = await tools[0].execute("call-search", {
    toolkit: "gmail",
    query: "send my professor an email",
  });
  assert.equal(search.details.tools[0].slug, "GMAIL_SEND_EMAIL");
  const result = await tools[1].execute("call-run", {
    toolkit: "gmail",
    toolSlug: "GMAIL_SEND_EMAIL",
    arguments: { recipient: "professor@example.edu" },
  });
  assert.deepEqual(calls, [
    { toolkit: "gmail", toolSlug: "GMAIL_SEND_EMAIL", arguments_: { recipient: "professor@example.edu" } },
  ]);
...
test("connected app failures are observable without exposing arguments", async () => {
  ...
  const [, execute] = await createConnectedAppTools(gateway, {
    observeExecution: (event) => observations.push(event),
  });
  await assert.rejects(
    () =>
      execute.execute("call-2", {
        toolkit: "notion",
        toolSlug: "NOTION_CREATE_PAGE",
        arguments: { title: "secret class notes" },
      }),
...
test("connected app tools stay absent when the server or every account is unavailable", async () => {
  assert.deepEqual(
    await createConnectedAppTools({ connectedApps: async () => ({ configured: false, toolkits: [] }) }),
    [],
  );
  assert.deepEqual(
    await createConnectedAppTools({
      ...
      searchConnectedAppTools: async () => {
        throw new Error("disconnected toolkits must not search");
      },
    }),
    [],
  );
