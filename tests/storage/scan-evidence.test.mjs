...
class Browser {
  ...
  async snapshot() {
    return {
      ...
      elements: [
        {
          ref: `r${this.revision}:1`,
          role: "heading",
          name: this.url === courseUrl ? "Calculus" : "Puzzle Game",
        },
      ],
...
async function recordReady(tools, browser) {
  ...
  return invoke(tools, "scan_record_assignment", {
    ...
    requirementExcerpts: [
      { text: "Build a playable game." },
      { text: "Submit a README PDF with walkthroughs." },
    ],
...
async function finishPartial(tools) {
  return invoke(tools, "scan_finish", {
    coverage: [
      { target: "Course: Calculus", status: "partial", failure: "Other class sources still need checking." },
    ],
    navigationHints: [],
  });
}

test("visible IANA deadlines retain exact precision and reject unsupported model dates", async () =>
  fixture(async ({ scan, runtime, browser, store }) => {
    let recorded;
    runtime.next = async (tools) => {
      ...
      recorded = await invoke(tools, "scan_record_assignment", {
        ...input,
        dueAt: "2026-09-14T23:59:00-04:00",
      });
      ...
      for (const wrong of [
        ...
        "2026-09-14T11:59:00-04:00",
      ]) {
        await assert.rejects(
          invoke(tools, "scan_record_assignment", { ...input, dueAt: wrong }),
          /does not match the visible due-date text/,
        );
      ...
      assert.equal(
        derived.dueAt,
        recorded.dueAt,
        "an omitted model timestamp uses the visible named zone, not the computer zone",
      );
      await assert.rejects(
        invoke(tools, "scan_record_assignment", {
          ...input,
          dueText: dueText.replace("September 14", "September 15"),
        }),
        /claimed assignment due date/,
      );
    ...
    assert.ok(
      result.scan.failures.includes("Other class sources still need checking."),
      result.scan.failures.join("; "),
    );
    assert.equal(store.assignments.get(recorded.assignmentId).deadlinePrecision, "datetime");
  }));

test("a scoped details check refreshes only its assignment and waits for an explicit work request", async () =>
  fixture(async ({ scan, runtime, browser, store, manager }) => {
    let assignment;
    runtime.next = async (tools) => {
    ...
    store.assignments.put({
      ...assignment,
      requirementsState: "partial",
      missingRequirements: ["README requirements"],
    });
    const unrelated = store.assignments.put({
      ...assignment,
      assignmentId: "unrelated",
      title: "Other assignment",
      sourceTarget: "https://school.example/mod/assign/view.php?id=999",
    ...
    runtime.next = async (tools) => {
      ...
      browser.detail();
      await assert.rejects(
        invoke(tools, "scan_record_course", { label: "Calculus" }),
        /cannot change school/,
      );
      await assert.rejects(
        invoke(tools, "scan_read_assignment", { assignmentId: unrelated.assignmentId }),
        /selected/,
      );
      await assert.rejects(
        invoke(tools, "scan_record_assignment", { courseId: assignment.courseId, title: "Other assignment" }),
        /only the selected assignment/,
      );
      ...
      browser.snapshot = async () => {
        ...
        return {
          ...
          elements: [
            ...page.elements,
            { ref: `r${page.revision}:2`, role: "link", name: "Puzzle rubric", href: rubricUrl },
          ],
        };
      };
      ...
      await invoke(tools, "scan_record_assignment", {
        courseId: assignment.courseId,
        title: assignment.title,
        requirementExcerpts: [{ text: "Provide a walkthrough for every level." }],
        requirementsComplete: true,
        missingRequirements: [],
      });
      await invoke(tools, "scan_record_source", {
        kind: "details",
        courseId: assignment.courseId,
        state: "checked",
      ...
      await invoke(tools, "scan_finish", {
        coverage: [{ target: `Assignment: ${assignment.title}`, status: "verified" }],
        navigationHints: [],
      });
    };
    ...
    assert.equal(
      assignmentWorkEligibility(store.assignments.get(assignment.assignmentId), now).eligible,
      true,
    );
    ...
    assert.equal(
      manager.state().entries.length,
      0,
      "even automatic mode cannot enqueue from a read-only check",
    );
...
test("scoped sign-in recovery survives coordinator restart and submitted work stays out of the queue", async () =>
  fixture(async ({ scan, runtime, browser, store, manager }) => {
    let assignment;
    runtime.next = async (tools) => {
      assignment = await recordReady(tools, browser);
    ...
    runtime.next = async (tools) => {
      await invoke(tools, "scan_request_handoff", {
        kind: "school_sign_in",
        reason: "Sign in to check this assignment.",
      });
    };
    ...
    try {
      runtime.next = async (tools) => {
        browser.detail();
        browser.text = browser.text.replace("Not submitted", "Submitted for grading");
        await invoke(tools, "scan_record_assignment", {
          courseId: assignment.courseId,
          title: assignment.title,
...
test("separate excerpts persist; date-only updates remove invented precision; false submission and late claims fail", async () =>
  fixture(async ({ scan, runtime, browser, store }) => {
    runtime.next = async (tools) => {
      const assignment = await recordReady(tools, browser);
      ...
      await assert.rejects(
        invoke(tools, "scan_record_assignment", {
          courseId: assignment.courseId,
          title: assignment.title,
          instructions: "Build a playable game. Submit a README PDF with walkthroughs.",
        }),
        /claimed assignment instructions/,
      );
      ...
      await assert.rejects(
        invoke(tools, "scan_record_assignment", {
          courseId: assignment.courseId,
          title: assignment.title,
          schoolStatus: { state: "not_submitted", text: "Submitted for grading" },
        }),
        /School status/,
      );
      await assert.rejects(
      ...
      await assert.rejects(
        invoke(tools, "scan_record_assignment", {
          courseId: assignment.courseId,
          title: assignment.title,
          latePolicy: { state: "accepted", text: "Late submissions are not accepted." },
        }),
        /non-contradictory/,
      );
      browser.text += "\nGrade: —\nGrade pending\nLate submissions accepted.\n2026-09-18T23:55:00.000Z";
      for (const text of ["Grade: —", "Grade pending"])
        await assert.rejects(
          invoke(tools, "scan_record_assignment", {
            courseId: assignment.courseId,
            title: assignment.title,
            schoolStatus: { state: "graded", text },
          }),
          /School status/,
        );
      await assert.rejects(
        invoke(tools, "scan_record_assignment", {
          ...
          latePolicy: {
            state: "accepted",
            text: "Late submissions accepted.",
            until: "2026-09-18T23:55:00.000Z",
            untilText: "2026-09-18T23:55:00.000Z",
          },
        }),
        /cutoff must occur/,
      );
      await assert.rejects(
        invoke(tools, "scan_record_assignment", {
          courseId: assignment.courseId,
          title: assignment.title,
          dueText: "October 13, 2026",
          dueAt: "2026-10-13T04:00:00.000Z",
        }),
        /date-only/,
      );
      const updated = await invoke(tools, "scan_record_assignment", {
        courseId: assignment.courseId,
        title: assignment.title,
        dueText: "October 13, 2026",
        schoolStatus: { state: "submitted", text: "Submitted for grading" },
      ...
      const replaced = await invoke(tools, "scan_record_assignment", {
        courseId: assignment.courseId,
        title: assignment.title,
        replaceRequirementsFromSource: true,
        requirementExcerpts: [{ text: "Build a playable game." }],
      });
      assert.equal(replaced.requirementEvidence.length, 1);
      assert.equal(
        replaced.requirementsState,
        "partial",
        "removing an old excerpt must not retain an old completeness claim",
      );
      await finishPartial(tools);
    };
...
test("manual discovery, explicit request, automatic withdrawal and restart keep distinct work intent", async () =>
  fixture(async ({ scan, runtime, browser, store, manager }) => {
    runtime.next = async (tools) => {
      await recordReady(tools, browser);
      await finishPartial(tools);
    };
    await scan.startScan();
    ...
    store.assignments.put({
      ...assignment,
      schoolStatus: { ...assignment.schoolStatus, state: "submitted", text: "Submitted for grading" },
    });
...
test("saved source checkpoint survives restart; unchanged requirements reuse; changed deadline is detected", async () =>
  fixture(async ({ scan, runtime, browser, store, manager }) => {
    let assignment;
    runtime.next = async (tools) => {
      assignment = await recordReady(tools, browser);
    ...
    runtime.next = async (tools) => {
      browser.detail();
      const reused = await invoke(tools, "scan_check_source", {
        kind: "details",
        courseId: assignment.courseId,
      });
      assert.equal(reused.reusable, true);
      assert.equal(
        store.assignments.get(assignment.assignmentId).schoolStatus.evidence.capturedAt,
        assignment.schoolStatus.evidence.capturedAt,
      );
      browser.text = browser.text.replace(due, "2026-09-15T23:59:00.000Z");
      assert.equal(
        (await invoke(tools, "scan_check_source", { kind: "details", courseId: assignment.courseId }))
          .unchanged,
        false,
      );
      browser.detail();
      ...
      assert.equal(
        status.assignments[0].requirementEvidence,
        undefined,
        "checkpoint summary does not repeat all instructions",
      );
      assert.equal(
        (await invoke(tools, "scan_read_assignment", { assignmentId: assignment.assignmentId })).assignment
    ...
    try {
      const result = await resumed.resume();
      assert.equal(result.scan.scanId, first.scan.scanId);
      assert.equal(result.scan.state, "partial");
    } finally {
      resumed.dispose();
    }
...
test("source budget rotates sessions from saved progress and stops honestly at a bounded run limit", async () =>
  fixture(async ({ scan, runtime, browser, store, manager }) => {
    scan.dispose();
    const bounded = new SchoolScanCoordinator(store, runtime, browser, {
      manager,
      now: () => now,
      maxSourcesPerSession: 1,
      maxSessionsPerRun: 2,
    });
    runtime.next = async (tools) => {
      if (runtime.sessions === 1) {
        ...
        await invoke(tools, "scan_record_source", {
          kind: "details",
          courseId: status.assignments[0].courseId,
          state: "checked",
          assignmentIds: [status.assignments[0].assignmentId],
        });
      }
    };
    try {
...
test("a list cannot attach another assignment's requirements to the selected assignment", async () =>
  fixture(async ({ scan, runtime, browser }) => {
    runtime.next = async (tools) => {
      const assignment = await recordReady(tools, browser);
      browser.url = courseUrl;
      ...
      browser.snapshot = async () => ({
        ...(await original()),
        elements: [{ ref: "link", role: "link", name: "Puzzle Game", href: assignmentUrl }],
      });
      await assert.rejects(
        invoke(tools, "scan_record_assignment", {
          courseId: assignment.courseId,
          title: assignment.title,
          requirementExcerpts: [{ text: "write a lab report" }],
          requirementsComplete: true,
        }),
        /detail page|row/,
      );
...
test("eligibility excludes completed, stale, incomplete and overdue work; extension must still be open", async () =>
  fixture(async ({ scan, runtime, browser }) => {
    runtime.next = async (tools) => {
      const ready = await recordReady(tools, browser);
      for (const state of ["unknown", "submitted", "graded", "locked"])
        assert.equal(
          assignmentWorkEligibility({ ...ready, schoolStatus: { ...ready.schoolStatus, state } }, now)
            .eligible,
          false,
        );
      assert.equal(assignmentWorkEligibility(ready, "2026-09-14T00:00:00.000Z").eligible, false);
      assert.equal(
        assignmentWorkEligibility({ ...ready, requirementsState: "partial" }, now).eligible,
        false,
      );
      ...
      const extended = {
        ...
        latePolicy: {
          state: "accepted",
          text: "Late submissions accepted until September 15",
          until: "2026-09-15T23:59:00.000Z",
          evidence: ready.deadlineEvidence,
        },
      };
      assert.equal(assignmentWorkEligibility(extended, now).eligible, true);
      assert.equal(
        assignmentWorkEligibility({ ...extended, latePolicy: { ...extended.latePolicy, until: now } }, now)
          .eligible,
        false,
      );
      await finishPartial(tools);
...
test("list deadlines cannot be borrowed from another assignment", async () =>
  fixture(async ({ scan, runtime, browser }) => {
    runtime.next = async (tools) => {
      const assignment = await recordReady(tools, browser);
      browser.url = courseUrl;
...
test("fresh status and deadline cannot revive stale attachment instructions", async () =>
  fixture(async ({ scan, runtime, browser }) => {
    runtime.next = async (tools) => {
      const ready = await recordReady(tools, browser);
      const stale = {
        ...ready,
        requirementEvidence: ready.requirementEvidence.map((item) => ({
          ...item,
          evidence: { ...item.evidence, kind: "document", capturedAt: "2026-08-01T00:00:00.000Z" },
        })),
      };
