...
test("addressed sends keep home separate and resume one assignment job across restart", async () => {
  ...
  try {
    ...
    assert.equal(
      conversations.state({ kind: "assignment", assignmentId: "assignment-statistics" }).job.jobId,
      first.job.jobId,
    );
    ...
    assert.equal(
      conversations.state().job.messages.length,
      home.job.messages.length,
      "stopping an assignment leaves home history intact",
    );
...
test("one home conversation keeps stable refs, deduplicates delivery and survives restart", async () => {
  const root = await mkdtemp(join(tmpdir(), "studi-chat-delivery-"));
  let store, manager, chat;
  try {
    store = await openLocalStore(root);
    const runtime = new FakeAgentRuntime();
    manager = await ManagerCoordinator.create(store, runtime);
    ...
    store.assignments.put({
      ...
      sourceTarget: "https://school.example.edu/math",
      discoveredAt: "2026-09-03T12:00:00.000Z",
      lastVerifiedScanId: "scan-1",
      evidence: [],
    });
    const metadata = {
      ...
      assignmentRefs: [{ assignmentId: "math", title: "Forged label" }],
    };
    const first = await chat.send({ kind: "home" }, "Explain this", metadata);
    const duplicate = await chat.send({ kind: "home" }, "Explain this", metadata);
    assert.equal(duplicate.job.messages.length, 2);
    assert.equal(duplicate.job.jobId, first.job.jobId);
    assert.equal(first.job.messages[0].assignmentRefs[0].title, "Canonical title");
    assert.equal(chat.state().activity, "idle");
    assert.equal(chat.isBusy, false);
    await assert.rejects(
      chat.send({ kind: "home" }, "Deleted ref", {
        assignmentRefs: [{ assignmentId: "deleted", title: "Missing" }],
      }),
      /no longer/,
    );
    assert.equal(chat.state().job.messages.length, 2);
    chat.dispose();
    manager.dispose();
    store.close();
    store = await openLocalStore(root);
    manager = await ManagerCoordinator.create(store, runtime);
    chat = new ConversationCoordinator(store, runtime, manager);
    assert.equal(chat.state().job.jobId, first.job.jobId);
    assert.deepEqual(chat.state().job.messages, first.job.messages);
  } finally {
    chat?.dispose();
    manager?.dispose();
    store?.close();
...
test("stop during session setup aborts once; failed replies remain recoverable and observable", async () => {
  const root = await mkdtemp(join(tmpdir(), "studi-chat-stop-"));
  let store, manager, chat;
  try {
    store = await openLocalStore(root);
    const base = new FakeAgentRuntime();
    manager = await ManagerCoordinator.create(store, base);
    let release;
    let began;
    const entered = new Promise((r) => (began = r));
    const gate = new Promise((r) => (release = r));
    const runtime = {
      createJobSession: async (...args) => {
        began();
        await gate;
        return base.createJobSession(...args);
      },
    };
    chat = new ConversationCoordinator(store, runtime, manager);
    const pending = chat.send({ kind: "home" }, "Wait for me");
    await entered;
    assert.equal(chat.state().activity, "thinking");
    await chat.stop();
    release();
    const stopped = await pending;
    assert.equal(stopped.outcome, "aborted");
    assert.equal(stopped.job.messages[1].recovery, "aborted");
    assert.equal(chat.isBusy, false);
    chat.dispose();
    chat = new ConversationCoordinator(
      store,
      {
        createJobSession: async () => {
          throw new Error("Provider disconnected");
        },
      },
      manager,
    );
    const failed = await chat.send({ kind: "home" }, "Try this");
    assert.equal(failed.outcome, "failed");
    assert.equal(failed.job.messages.at(-1).recovery, "failed");
    assert.equal(chat.state().activity, "idle");
    assert.ok(chat.trace.events().some((e) => e.type === "error"));
  } finally {
    chat?.dispose();
    manager?.dispose();
    store?.close();
...
test("account home histories stay separate and interrupted acceptance recovers once", async () => {
  const root = await mkdtemp(join(tmpdir(), "studi-chat-account-"));
  let store, manager, chat;
  try {
    store = await openLocalStore(root);
    const runtime = new FakeAgentRuntime();
    manager = await ManagerCoordinator.create(store, runtime);
    chat = new ConversationCoordinator(store, runtime, manager, { ownerSubject: "qa-one" });
    const one = await chat.send({ kind: "home" }, "My message");
    chat.dispose();
    chat = new ConversationCoordinator(store, runtime, manager, { ownerSubject: "qa-two" });
    assert.notEqual(chat.state().job.jobId, one.job.jobId);
    assert.equal(chat.state().job.messages.length, 0);
    const job = chat.state().job;
    store.agentJobs.put({
      ...job,
      turnIndex: 1,
      messages: [
    ...
    assert.equal(chat.state().job.messages.length, 2);
    assert.equal(chat.state().job.messages.length, 2);
    assert.equal(chat.state().job.messages.at(-1).recovery, "failed");
    chat.dispose();
    chat = new ConversationCoordinator(store, runtime, manager, { ownerSubject: "qa-one" });
    assert.deepEqual(chat.state().job.messages, one.job.messages);
  } finally {
    chat?.dispose();
    manager?.dispose();
    store?.close();
...
test("home status includes the current saved answer and actual review phase", async () => {
  ...
  let homeTools;
  const runtime = {
    createJobSession: async (target, tools, sessionTarget) => {
      homeTools = tools;
  ...
  try {
    store.lifecycle.putExecution({
      ...
      turnCount: 1,
      attemptCount: 0,
      answerSnapshot: "Rain tapped on the yellow umbrella.",
      reviewDeadline: "2026-09-07T12:15:00.000Z",
      reviewCheckpoint: {
        revision: 1,
    ...
    const status = await homeTools
      .find((tool) => tool.name === "home_status")
      .execute("status", {}, undefined, undefined, {});
