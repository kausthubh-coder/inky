...
test('download preserves exact bytes and duplicate names, then reads text and image-only PDF pages', async () => {
  ...
  try {
    ...
    const browser = {
      downloadSource: async ref => { assert.equal(ref, 'fresh-link'); return 'https://school.test/download.php'; },
      fetchDownload: async () => new Response(schoolPdf(), { headers: { 'content-type': 'application/pdf', 'content-disposition': "attachment; filename*=UTF-8''Exercise%206.pdf" } }),
    };
...
test('HTML login, HTTP errors, oversized and interrupted streams leave no files; a later download succeeds', async () => {
  ...
  try {
    ...
    for (const [response, message] of [
      ...
      [() => new Response(new ReadableStream({ start(c) { c.enqueue(new Uint8Array(50_000_001)); c.close(); } })), /50 MB/],
      [() => new Response(new ReadableStream({ start(c) { c.enqueue(new Uint8Array([1])); c.error(new Error('connection interrupted')); } })), /interrupted/],
    ]) {
      const browser = { downloadSource: async () => 'https://school.test/file', fetchDownload: async () => response() };
    ...
    const blocked = { downloadSource: async () => 'https://school.test/file', fetchDownload: async () => { throw new Error('must not fetch'); } };
    await assert.rejects(createBrowserDownloadTool(blocked, files).execute('cancel', {}, abort.signal), /abort/i);
    const redirected = {
