...
test("dashboard handoff resumes the same replay without navigating away from the student's page", async () => {
  ...
  try {
    ...
    const note = {
      scanId: paused.scan.scanId,
      text: "Check every class before finishing.",
      clientMessageId: "00000000-0000-4000-8000-000000000011",
    };
    await coordinator.sendMessage(note);
    await coordinator.sendMessage(note);
    assert.equal(
      (await coordinator.state()).scan.messages.length,
      1,
      "retrying a paused message does not duplicate it",
    );
...
test("school scan pauses for sign-ins, records evidence, replays from root, and preserves prior rows on partial and zero results", async () => {
  ...
  try {
    ...
    const runtime = new ScriptedScanRuntime([
      ...
      async (tools) => {
        browser.showAssignments();
        const course = await invoke(tools, "scan_record_course", {
          label: "Calculus",
          courseKey: "calc-101",
        });
      ...
      async (tools) => {
        ...
        await invoke(tools, "scan_finish", {
          coverage: [
            { target: "Assignment lists", status: "partial", failure: "One course page timed out." },
          ],
          navigationHints: [],
        });
      },
    ...
    const assignmentTasks = store.tasks
      .listAll()
      .filter((task) => task.assignmentId === assignment.assignmentId);
    assert.equal(assignmentTasks.length, 1, "a verified assignment has one durable task origin");
    assert.equal(
      manager.state().entries.length,
      0,
      "permission alone does not queue discoveries in manual work mode",
    );
    ...
    const navigationNote = await store.notes.read(
      store.notes.list({ scope: "school", about: "scan" })[0].noteId,
    );
    ...
    assert.equal(
      state.assignments[0].lastVerifiedScanId,
      state.scan.scanId,
      "replay refreshed the assignment from a current fact",
    );
    assert.equal(
      store.tasks.listAll().filter((task) => task.assignmentId === assignment.assignmentId).length,
      1,
      "replay does not duplicate the task origin",
    );
    ...
    assert.equal(
      ...
      1,
      "a correction is a note and does not mutate the executable workflow",
    );
    assert.deepEqual(state.profile.missedCourseFeedback, ["Also look for the honors seminar."]);
    const correction = store.notes
      .list({ scope: "school", subjectId: "primary-school", about: "scan" })
    ...
    assert.equal(
      databaseBytes.includes(Buffer.from("SECRET_PAGE_HTML")),
      false,
      "page text never enters SQLite",
    );
  } finally {
    coordinator?.dispose();
    manager?.dispose();
...
test("a caught scan failure reports the original cause without letting telemetry break recovery", async () => {
  ...
  const reports = [];
  const coordinator = new SchoolScanCoordinator(
    store,
    new ScriptedScanRuntime([
      async () => {
        throw failure;
      },
    ]),
...
test("unreleased Gradescope assignments verify an empty source without hiding login or course gaps", async () => {
  ...
  const runtime = new ScriptedScanRuntime(
    [false, true].map((hasGap) => async (tools) => {
      browser.showAssignments();
            ...
            ? [
                {
                  ...
                  status: "partial",
                  failure: "Workshop instructions are blocked by an unfinished prerequisite.",
                },
              ]
            : []),
        ],
  ...
  try {
    ...
    assert.deepEqual(partial.scan.failures, [
      "Workshop instructions are blocked by an unfinished prerequisite.",
    ]);
  } finally {
    coordinator.dispose();
    store.close();
...
test("Moodle list → detail → changed course label/key → partial replay keeps one assignment and task", async () => {
  ...
  const runtime = new ScriptedScanRuntime(
    [0, 1, 2, 3].map((turn) => async (tools) => {
      ...
      await assert.rejects(
        invoke(tools, "scan_record_assignment", { courseId: course.courseId, title: "Homework 1" }),
        /no unambiguous assignment link/,
      );
      ...
      browser.elements =
        turn % 2 === 0
          ? [
              { ref: "homework", role: "link", name: "Homework 1", href: destination },
              {
                ref: "homework-row",
                role: "row",
                name: "Homework 1 Write a C program. 2026-09-09T12:00:00.000Z",
              },
            ]
          : [];
      await invoke(tools, "scan_record_assignment", {
        courseId: course.courseId,
        title: "Homework 1",
        assignmentKey: `model-assignment-${turn}`,
        ...(turn === 0
          ? {
              dueAt: "2026-09-09T12:00:00.000Z",
              dueText: "2026-09-09T12:00:00.000Z",
              instructions: "Write a C program.",
            }
          : {}),
      });
      await recordFixtureInventories(tools, browser);
      await invoke(tools, "scan_finish", {
        coverage: [
          {
            target: `Course: ${label}`,
            status: turn === 3 ? "partial" : "verified",
            ...(turn === 3 ? { failure: "Another page timed out" } : {}),
          },
        ],
        navigationHints: [],
      });
  ...
  const runtime = new ScriptedScanRuntime([
    async (tools) => {
      ...
      store.school.putCourse({
        ...course,
        courseId: "old-long-name",
        label: "CSC 230 (002) Fall 2026 C and Software Tools",
      });
      store.assignments.put({
        ...original,
        assignmentId: "legacy-detail",
        sourceTarget: destination,
        courseId: "old-long-name",
      });
      store.assignments.put({
        ...original,
        assignmentId: "legacy-detail-again",
        sourceTarget: `${destination}&action=view`,
      });
      const input = { courseId: course.courseId, title: "Homework 1" };
      await assert.rejects(
        invoke(tools, "scan_record_assignments", {
          assignments: [input, { ...input, title: "Invented homework" }],
        }),
        /claimed assignment title/,
      );
      ...
      assert.equal(store.tasks.listAll().length, 1);
      assert.equal(
        store.assignments.get("legacy-list").assignmentId,
        store.assignments.get("legacy-detail").assignmentId,
      );
      assert.equal(
        store.assignments.get("legacy-list").assignmentId,
        store.assignments.get("legacy-detail-again").assignmentId,
      );
      await recordFixtureInventories(tools, browser);
      await invoke(tools, "scan_finish", {
        coverage: [{ target: "Course: C and Software Tools", status: "verified" }],
        navigationHints: [],
      });
    },
  ]);
...
test("existing course aliases and exercise_05 survive list/detail/rescan/restart as one class, assignment and task", async () => {
  ...
  const short = "C and Software Tools",
    full = "CSC 230 (002) Fall 2026 C and Software Tools";
  for (const [courseId, label, sourceTarget] of [
    ["legacy-short", short, rootUrl],
    ["legacy-full", full, list],
  ]) {
    store.school.putCourse({
      schemaVersion: 1,
      courseId,
      label,
      sourceTarget,
      lastVerifiedScanId: "old-scan",
      lastVerifiedAt: now,
      evidence: {
        schemaVersion: 1,
        evidenceId: `evidence-${courseId}`,
        reference: `evidence-${courseId}`,
  ...
  for (const [assignmentId, courseId, sourceTarget] of [
    ["legacy-list", "legacy-short", list],
    ["legacy-detail", "legacy-full", destination],
  ]) {
    store.assignments.put({
      schemaVersion: 1,
      assignmentId,
  ...
  store.lifecycle.putExecution({
    schemaVersion: 1,
    taskId: "task-legacy-detail",
    assignmentId: "legacy-detail",
    phase: "preserved",
    answerArtifactId: "answer-exercise",
    taskBudget: { maxAgentTurns: 24, maxRecoveryAttempts: 2 },
    turnCount: 1,
    attemptCount: 0,
    answerSnapshot: "My saved C program",
    updatedAt: now,
  });
  await store.artifacts.write({
    frontmatter: { schemaVersion: 1, kind: "answer", artifactId: "answer-exercise", updatedAt: now },
    content: "My saved C program for task-legacy-detail",
  });
  ...
  assert.equal(
    store.assignments.listAll().length,
    2,
    "startup waits for verified assignment destination evidence",
  );
  const runtime = new ScriptedScanRuntime(
    [0, 1, 2, 3].map((turn) => async (tools) => {
      ...
      browser.elements =
        turn === 1 ? [] : [{ ref: "exercise", role: "link", name: "exercise_05", href: destination }];
      if (turn === 0) {
        browser.url = list.replace("11497", "12172");
        await assert.rejects(
          invoke(tools, "scan_record_assignment", { courseId: "legacy-full", title: "exercise_05" }),
          /different course/,
        );
        assert.equal(store.assignments.listAll().length, 2);
        browser.url = list;
      }
      const recorded = await invoke(tools, "scan_record_assignment", {
        courseId: "legacy-full",
        title: "exercise_05",
        assignmentKey: `key-${turn}`,
      });
      assert.equal(recorded.assignmentId, "legacy-detail");
      assert.equal(recorded.courseId, course.courseId);
      await recordFixtureInventories(tools, browser);
      await invoke(tools, "scan_finish", {
        coverage: [{ target: `Course: ${label}`, status: "verified" }],
        navigationHints: [],
      });
    }),
  );
  ...
  const runtime = new ScriptedScanRuntime([
    async (tools) => {
      ...
      const input = {
        courseId: course.courseId,
        title: "Limits practice",
        assignmentKey: "limits",
        observationRef: "r8:2",
        dueAt: "2026-09-03T15:00:00.000Z",
        dueText: "2026-09-03T15:00:00.000Z",
        instructions: "Write three sentences about limits.",
      };
      ...
      const dated = await invoke(tools, "scan_record_assignment", {
        ...input,
        dueAt: undefined,
        dueText: "2026-09-09 at 11:59 PM",
      });
      assert.equal(dated.dueAt, new Date("2026-09-09 11:59 PM").toISOString());
      assert.equal(dated.dueText, "2026-09-09 at 11:59 PM");
      await assert.rejects(
        invoke(tools, "scan_record_assignment", { ...input, instructions: "Invented instructions" }),
        /claimed assignment instructions/,
      );
      await assert.rejects(
        invoke(tools, "scan_record_course", { label: "Invented course", observationRef: "r1:1" }),
        /claimed course label/,
      );
      await recordFixtureInventories(tools, browser);
      await invoke(tools, "scan_finish", {
        coverage: [{ target: "Course: Calculus", status: "verified" }],
...
test("replay preserves course, assignment and task identities when the model changes suggested keys", async () => {
  ...
  const browser = new RecordingBrowser();
  const runtime = new ScriptedScanRuntime(
    ["first-key", "different-key"].map((key) => async (tools) => {
      browser.showAssignments();
...
test("a running check delivers a steering message once and preserves concurrent discoveries", async () => {
  ...
  const finishTurn = new Promise((resolve) => {
    finish = resolve;
  });
  ...
  runtime.createScanSession = async (tools) => ({
    ...(await create(tools)),
    steer: async (text) => {
      calls.push(text);
      await delivery;
  ...
  try {
    ...
    const input = {
      scanId: (await scan.state()).scan.scanId,
      text: "Check every class",
    ...
    finish();
    assert.equal((await running).scan.state, "succeeded");
  } finally {
    delivered?.();
    finish?.();
    scan.dispose();
  ...
  const runtime = new ScriptedScanRuntime(
    [0, 1].map((turn) => async (tools) => {
      ...
      browser.elements = turn
        ? [
            { ref: "course", role: "link", name: "Calculus", href: rootUrl + "courses/calculus" },
            {
              ref: "assignment",
              role: "link",
              name: "Limits practice",
              href: rootUrl + "assignments/limits",
            },
      ...
      await invoke(tools, "scan_record_assignment", {
        courseId: course.courseId,
        title: "Limits practice",
        ...(turn ? { dueAt: "2026-09-03T15:00:00.000Z", dueText: "2026-09-03T15:00:00.000Z" } : {}),
      });
      await recordFixtureInventories(tools, browser);
      await invoke(tools, "scan_finish", {
        coverage: [{ target: "Course: Calculus", status: "verified" }],
...
async function recordFixtureInventories(tools, browser) {
  ...
  try {
    browser.url = rootUrl;
    browser.elements = [];
    browser.text = "All courses: " + courses.map((course) => course.label).join(", ");
    await invoke(tools, "scan_record_inventory", {
      kind: "courses",
      state: "complete",
      itemIds: scan.observedCourseIds,
      evidenceText: "All courses",
    });
    for (const course of courses) {
      const items = assignments.filter((item) => item.courseId === course.courseId);
      browser.url = course.sourceTarget;
      browser.text =
        course.label +
        " " +
        (items.length ? "All assignments: " + items.map((item) => item.title).join(", ") : "No assignments");
      await invoke(tools, "scan_record_inventory", {
        kind: "assignments",
        courseId: course.courseId,
        state: items.length ? "complete" : "empty",
        itemIds: items.map((item) => item.assignmentId),
        evidenceText: items.length ? "All assignments" : "No assignments",
      });
    }
  } finally {
...
test("school check reports only new or changed work and refuses incomplete inventory", async () => {
  const root = await mkdtemp(join(tmpdir(), "studi-scan-changes-"));
  const store = await openLocalStore(root);
  const browser = new RecordingBrowser();
  const runtime = new ScriptedScanRuntime(
    [0, 1, 2, 3].map((turn) => async (tools) => {
      ...
      browser.text = "Calculus Limits practice " + due;
      browser.elements = [];
      await invoke(tools, "scan_record_assignment", {
        courseId: course.courseId,
        title: "Limits practice",
  ...
  try {
    ...
    const first = await coordinator.startScan();
    assert.equal(first.scan.changes.length, 1);
    assert.equal(first.scan.changes[0].kind, "new");
    const unchanged = await coordinator.replay();
    assert.equal(unchanged.scan.changes.length, 0);
    const updated = await coordinator.replay();
    assert.equal(updated.scan.changes[0].kind, "updated");
    assert.ok(updated.scan.changes[0].fields.includes("dueAt"));
    assert.equal(updated.assignments.length, 1);
    assert.deepEqual(
      updated.scan.changes[0].dueChange,
      {
    ...
    try {
      assert.deepEqual(reopened.school.latestScan().changes, updated.scan.changes);
    } finally {
      reopened.close();
    }
    const incomplete = await coordinator.replay();
    assert.equal(incomplete.scan.state, "partial");
