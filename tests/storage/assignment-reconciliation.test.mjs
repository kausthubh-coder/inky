...
import {
  ...
  schoolIdentity,
  observedTarget,
} from "../../dist/electron/scan/source-identity.js";
...
function seed(store, id, sourceTarget = target, courseId = "course-short") {
  const assignment = store.assignments.put({
    schemaVersion: 1,
    assignmentId: id,
    courseId,
    title: "Homework 1",
...
test("alias merge retains legacy deadlines and newer status while preserving requirement gaps", async () =>
  fixture(async (store) => {
    ...
    store.assignments.put({
      ...
      dueText: "September 10, 2026 11:59 PM",
      schoolStatus: { state: "not_submitted", text: "Not submitted", evidence: first },
      requirementEvidence: [{ text: "Write a program.", evidence: first }],
      requirementsState: "complete",
    });
    store.assignments.put({
      ...donor,
      schoolStatus: { state: "submitted", text: "Submitted for grading", evidence: later },
      requirementEvidence: [{ text: "Include the test report.", evidence: later }],
      requirementsState: "partial",
      missingRequirements: ["Read the rubric."],
    });
    ...
    assert.equal(
      ...
      "legacy deadline is not erased by an undated donor",
    );
...
test("equally recent contradictory school facts keep aliases blocked instead of choosing an unfinished state", async () =>
  fixture(async (store) => {
    const a = seed(store, "a");
    const b = seed(store, "b");
    store.assignments.put({
      ...a,
      schoolStatus: { state: "not_submitted", text: "Not submitted", evidence: factEvidence("a-status") },
    });
    store.assignments.put({
      ...b,
      schoolStatus: { state: "submitted", text: "Submitted for grading", evidence: factEvidence("b-status") },
    });
    ...
    try {
      assert.equal(manager.resolvePermission("a", a.courseId).mayAttempt, false);
    } finally {
      manager.dispose();
    }
  }));

test("a student-requested queue entry chooses the retained alias and keeps its intent", async () =>
  fixture(async (store) => {
    for (const id of ["a", "b"]) {
      ...
      store.assignments.put({
        ...
        requirementEvidence: [{ text: "Write a program.", evidence }],
        requirementsState: "complete",
      });
    }
...
test("school identity ignores Moodle view options, but separates installations, module types and ids", () => {
  assert.equal(
    schoolIdentity(target, "assignment"),
    schoolIdentity(`${target}&action=editsubmission#intro`, "assignment"),
  );
  for (const other of [
    target.replace("1360376", "1360377"),
    target.replace("school.", "other."),
    target.replace("/moodle/", "/another/"),
    target.replace("/assign/", "/quiz/"),
  ]) {
    assert.notEqual(schoolIdentity(target, "assignment"), schoolIdentity(other, "assignment"));
  }
  assert.equal(
    schoolIdentity("https://school.example.edu/moodle/course/view.php?id=123", "course"),
    schoolIdentity("https://school.example.edu/moodle/mod/assign/index.php?id=123", "course"),
  );
  assert.equal(
    schoolIdentity("https://school.example.edu/moodle/mod/assign/index.php?id=123", "assignment"),
    null,
  );
  assert.equal(schoolIdentity(`${target}&id=999`, "assignment"), null);
  assert.notEqual(
    assignmentIdentity("https://school.example.edu/#/assignments/1"),
    assignmentIdentity("https://school.example.edu/#/assignments/2"),
  );
});
...
test("repair retains the worked task, archives originals, redirects old links, and survives backup/restart", async () => {
  await fixture(async (store, root) => {
    ...
    store.lifecycle.putExecution({
      schemaVersion: 1,
      taskId: "task-b-worked",
      assignmentId: worked.assignmentId,
      phase: "preserved",
      answerArtifactId: "answer-b-worked",
      taskBudget: { maxAgentTurns: 24, maxRecoveryAttempts: 2 },
      turnCount: 1,
      attemptCount: 0,
      answerSnapshot: "My saved answer",
      updatedAt: now,
    });
    await store.artifacts.write({
      frontmatter: { schemaVersion: 1, kind: "answer", artifactId: "answer-b-worked", updatedAt: now },
      content: "My saved answer for task-b-worked",
    });
    ...
    const archived = JSON.parse(
      store.database.handle.prepare("SELECT record_json FROM record_redirects WHERE kind = 'task'").get()
        .record_json,
    );
    assert.equal(archived.events[0].payload.assignmentId, "a-empty");
    assert.throws(() => seed(store, "a-empty"), /merged/);
    const rule = store.permissionRules.put({
      schemaVersion: 1,
      ruleId: "old-link-rule",
      scope: "assignment",
    ...
    assert.equal(
      rule.assignmentId,
      "b-worked",
      "a restriction saved through an old assignment link applies to the retained assignment",
    );
    assert.deepEqual(reconcileAssignments(store), []);
    ...
    try {
      assert.equal(reopened.assignments.get("a-empty").assignmentId, "b-worked");
    } finally {
      reopened.close();
    }
  });
...
test("different permissions and separate work histories remain explicit conflicts and cannot enqueue", async () => {
  await fixture(async (store) => {
    ...
    const manager = await ManagerCoordinator.create(store, {}, { now: () => now });
    try {
      assert.throws(() => manager.enqueue({ taskId: "task-a" }), /blocked/);
    } finally {
      manager.dispose();
    }
    store.permissionRules.put({
...
test("repair rolls back its archives and task deletions if a write fails", async () => {
  await fixture(async (store) => {
    seed(store, "a");
    seed(store, "b");
    store.database.handle.exec(
      "CREATE TRIGGER reject_repair BEFORE DELETE ON assignments BEGIN SELECT RAISE(ABORT, 'repair failure'); END",
    );
