...
function course(store, id, target = list, label = "C and Software Tools") {
  return store.school.putCourse({
    ...
    evidence: {
      ...
      sourceTarget: target,
      capturedAt: now,
      summary: `Observed ${label}`,
    },
  });
}
function assignment(store, id, courseId, target = list) {
  return store.assignments.put({
    schemaVersion: 1,
    assignmentId: id,
    courseId,
    title: "exercise_05",
...
test("legacy directory and full-label class reconcile through course ID, preserving rules, assignments and old links", async () => {
  await fixture(async (store, root) => {
    ...
    assert.equal(
      ...
      2,
      "a shared title does not prove the list observation's assignment identity",
    );
    ...
    try {
      assert.equal(reopened.school.listCourses().length, 1);
      assert.equal(reopened.school.resolveCourseId(donor), canonical);
    } finally {
      reopened.close();
    }
  });
...
test("course policies remain separate and pause work, including inherited versus explicit permissions", async () => {
  await fixture(async (store) => {
    ...
    try {
      assert.equal(manager.resolvePermission("homework", "two").mayAttempt, false);
    } finally {
      manager.dispose();
    }
  });
...
test("two independent course notes remain intact for review; transaction failure rolls back redirects and membership", async () => {
  await fixture(async (store) => {
    ...
    store.database.handle.exec(
      "CREATE TRIGGER reject_course_repair BEFORE DELETE ON courses BEGIN SELECT RAISE(ABORT, 'repair failure'); END",
    );
    ...
    for (const id of ["one", "two"])
      await store.artifacts.write({
        frontmatter: { schemaVersion: 1, kind: "memory", artifactId: `memory-${id}`, updatedAt: now },
        content: `Saved notes for ${id}`,
      });
