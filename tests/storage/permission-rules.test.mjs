...
const rule = (ruleId, target, mode = "attempt", updatedAt = now) => ({
  schemaVersion: 1,
  ruleId,
  ...target,
  mode,
...
function legacyInsert(store, value) {
  store.database.handle
    .prepare(`INSERT INTO permission_rules
...
test("saving the same target replaces its permission across every scope and survives reopening", async () => {
  await fixture(async (store, root) => {
    ...
    assert.equal(
      store.database.handle.prepare("SELECT count(*) AS n FROM permission_rules").get().n,
      targets.length,
    );
...
test("legacy duplicates display the existing winner; deleting it cannot resurrect an older permission", async () => {
  await fixture(async (store) => {
    const values = [
      rule("old-allow", { scope: "global" }, "auto_submit", "2026-09-11T12:00:00.000Z"),
    ...
    assert.deepEqual(
      resolvePermission(context, store.permissionRules.listAll()),
      resolvePermission(context, values),
    );
...
test("replacement clears legacy duplicates atomically and a failed save preserves the current rule", async () => {
  await fixture(async (store) => {
    ...
    store.database.handle.exec(
      "CREATE TRIGGER fail_rule_save BEFORE INSERT ON permission_rules BEGIN SELECT RAISE(ABORT, 'save failure'); END",
    );
