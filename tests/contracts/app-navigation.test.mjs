...
test("chat links open externally while app navigation and unsafe destinations stay blocked", async () => {
  ...
  configureAppNavigation({
    ...
    on: (event, handler) => { assert.equal(event, "will-navigate"); navigate = handler; },
  }, async url => { opened.push(url); });

  const allowed = ["https://mail.google.com/mail/u/0/#inbox/example", "http://localhost:4174/", "mailto:teacher@example.com"];
  for (const url of [...allowed, "javascript:alert(1)", "file:///C:/secret.txt", "data:text/html,hello", "studi://auth", "/relative", "//example.com"]) {
