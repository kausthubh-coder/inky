...
const connection = (status) => ({
  toolkit: "gmail",
  sessionId: "test",
  connectedAccountId: status === "ACTIVE" ? "test" : null,
  status,
  redirectUrl: null,
});
const options = () => ({
  initial: connection("INITIATED"),
...
test("automatically waits through pending and delayed account creation until active", async () => {
  ...
  let calls = 0;
  const result = await waitForAppConnection({
    ...options(),
    read: async () => {
...
test("already active accounts finish without a refresh", async () => {
  await waitForAppConnection({
    ...
    read: () => assert.fail("unnecessary refresh"),
  });
});
...
test("terminal failures and failed requests end the attempt", async () => {
  for (const status of ["FAILED", "EXPIRED", "INACTIVE"]) {
    await assert.rejects(
      waitForAppConnection({ ...options(), read: async () => connection(status) }),
      /Connection failed/,
    );
  }
  await assert.rejects(
...
test("opening sign-in is also bounded by the connection deadline", async () => {
  await assert.rejects(
    waitForAppConnection({
      ...
      timeoutMs: 20,
      initial: new Promise(() => {}),
      read: () => assert.fail("sign-in never opened"),
    }),
    /timed out/,
  );
});
...
test("sign-out cancels a pending request and ignores its late result", async () => {
  ...
  const result = waitForAppConnection({
    ...options(),
