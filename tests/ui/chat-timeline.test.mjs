...
test("handoff cards stay between the earlier conversation and the student's later replies", () => {
  const messages = [
    {
      messageId: "before",
      createdAt: "2026-09-07T06:07:00Z",
      role: "user",
      text: "Scan my school",
      turnIndex: 0,
    },
    {
      messageId: "after",
      createdAt: "2026-09-07T06:09:00Z",
      role: "user",
      text: "I'm signed in",
      turnIndex: 1,
    },
    {
      messageId: "reply",
      createdAt: "2026-09-07T06:09:01Z",
      role: "assistant",
      text: "Checking",
      turnIndex: 1,
    },
  ...
  assert.deepEqual(
    ...
    ["before", "handoff", "after", "reply"],
  );
  assert.equal(timeline.at(-1).index, 2, "retry still points at the original conversation index");
  assert.deepEqual(
    chatTimeline(messages, cards),
    timeline,
    "polling does not move the card below later replies",
  );
...
test("assignment and review cards share the same chronological transcript", () => {
  ...
  assert.deepEqual(
    chatTimeline(messages, cards).map((entry) => entry.key),
    ["assignment", "question", "review"],
  );
});
