...
const { RendererTelemetry } = await import(
  `data:text/javascript;base64,${Buffer.from(bundled.outputFiles[0].text).toString("base64")}`
);
const state = {
  rendererConfig: { projectToken: "phc_test", host: "https://us.i.posthog.com" },
  enabled: true,
  replayEnabled: true,
...
function mockSdk() {
  ...
  const client = {
    init(_token, config) {
      options = config;
      optedOut = config.opt_out_capturing_by_default;
      id = config.bootstrap.distinctID;
    },
    opt_in_capturing() {
      optedOut = false;
      calls.push("opt-in");
...
test("renderer correlates rotated sessions, preserves ordinary replay content and respects opt-out", async () => {
  ...
  globalThis.window = {
    studi: {
      async captureUiTelemetry(value) {
        contexts.push(value);
        return true;
      },
    },
  ...
  try {
    ...
    assert.deepEqual(sdk.calls.at(-1), {
      event: "$pageview",
      props: { screen: "library", section: "workspace" },
    });
    const recording = sdk.options.session_recording;
    assert.equal(recording.maskInputOptions.password, true);
    assert.equal(
      recording.maskInputFn("PASSWORD_CANARY", { getAttribute: () => "password", closest: () => null }),
      "[secret]",
    );
    assert.equal(
      recording.maskInputFn("My exact prompt", { getAttribute: () => "text", closest: () => null }),
      "My exact prompt",
    );
    assert.equal(recording.maskTextFn("Math error token=TOKEN_CANARY"), "Math error [secret]");
    assert.equal(
      recording.maskInputFn("DEVICE_CODE", { getAttribute: () => "text", closest: () => ({}) }),
      "[secret]",
    );
...
test("renderer reports requested but unavailable replay once and retries a failed SDK load", async () => {
  ...
  try {
    ...
    assert.deepEqual(status, [
      { event: "studi_replay_status", props: { requested: true, recording: false } },
    ]);
  } finally {
    globalThis.window = previousWindow;
  }
