...
test("runtime diagnostics retain complete messages and tools, link model calls, and avoid streaming duplicates", () => {
  ...
  diagnostics.providerRequest("gpt-6-astra", "openai-codex", {
    instructions: "Scan my school",
    input: [{ role: "user", content: "Find Math homework" }],
  });
  ...
  diagnostics.accept({
    ...
    toolName: "browser_type",
    isError: false,
    result: { text: "School page" },
  });
  diagnostics.accept({
    type: "message_end",
    message: {
      role: "assistant",
      content: [{ type: "text", text: "Your Math homework is due Friday." }],
      stopReason: "stop",
  ...
  assert.equal(
    generation.payload.$ai_span_id,
    events.find((event) => event.kind === "provider_request").payload.span_id,
  );
  ...
  assert.equal(
    events.some((event) => event.kind === "message_update"),
    false,
  );
  assert.equal(
    events.find((event) => event.kind === "tool_execution_start").payload.args.text,
    "Exact answer",
  );
  assert.equal(
    events.find((event) => event.kind === "tool_execution_end").payload.result.text,
    "School page",
  );
  diagnostics.accept({ type: "agent_start" });
  assert.notEqual(events.at(-1).run_id, generation.run_id);
  const broken = new RuntimeDiagnostics("broken", () => {
